...
**Note:** The web feedback migration (messages.feedback → feedback collection + source tagging) runs automatically on Next.js app startup via `migrateWebFeedback()` in `ui/src/lib/mongodb.ts`. No manual step is needed for that.
...
### Step 1: Backfill Slack feedback from Langfuse
...
python scripts/migrations/0.3.0/backfill_slack_interactions.py --channels C010J2FQFLK,C07MU724GN9
...
# Dry run
python scripts/migrations/0.3.0/backfill_slack_interactions.py --from-json /tmp/slack_interactions.json --dry-run
```
