...
Search & Ingest grants permit content ingestion, not reassignment of the
...
@pytest.mark.asyncio
async def test_query_admin_can_update_only_owner_team(
  monkeypatch: pytest.MonkeyPatch,
  storage: AsyncMock,
) -> None:
  datasource = _datasource()
  ...
  query_admin_check = AsyncMock(return_value=None)
  management_check = AsyncMock(return_value=None)
  monkeypatch.setattr(restapi, "check_datasource_access", query_admin_check)
  ...
  query_admin_check.assert_awaited_once_with(ANY, "primary", "admin")
  management_check.assert_not_awaited()
...
async def test_source_manager_can_persist_independent_search_teams(
  ...
  storage.get_datasource_info.return_value = datasource
  monkeypatch.setattr(restapi, "check_datasource_access", AsyncMock(return_value=None))
  monkeypatch.setattr(
    restapi,
...
@pytest.mark.asyncio
async def test_query_admin_can_restore_an_unowned_datasource(
  monkeypatch: pytest.MonkeyPatch,
  storage: AsyncMock,
) -> None:
  datasource = _datasource()
...
async def test_source_manager_can_transfer_query_owner_without_content_access(
  ...
  monkeypatch.setattr(restapi, "check_datasource_access", deny_query_admin)
...
@pytest.mark.asyncio
async def test_query_owner_update_denies_caller_without_either_management_grant(
  monkeypatch: pytest.MonkeyPatch,
  storage: AsyncMock,
) -> None:
  ...
  storage.get_datasource_info.return_value = _datasource()
  monkeypatch.setattr(restapi, "check_datasource_access", deny)
...
@pytest.mark.asyncio
async def test_query_owner_update_rejects_invalid_team_slug(
  monkeypatch: pytest.MonkeyPatch,
  storage: AsyncMock,
) -> None:
  storage.get_datasource_info.return_value = _datasource()
