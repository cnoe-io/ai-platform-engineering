...
class TestRAGEndToEnd:
  ...
  def test_health_check(self):
    ...
    assert config.get("graph_rag_enabled") == self.graph_rag_enabled, f"Graph RAG mode mismatch: expected {self.graph_rag_enabled}, got {config.get('graph_rag_enabled')}"
  ...
  def test_url_ingestion_workflow(self):
    ...
    # Start ingestion
    ingest_data = {"url": test_url, "description": "Test HTML page for E2E testing", "default_chunk_size": 1000, "default_chunk_overlap": 100, "check_for_site_map": False, "sitemap_max_urls": 0}
    ...
    assert job_data["status"] in ["completed", "completed_with_errors"], f"Job failed: {job_data.get('message', 'Unknown error')}"
  ...
  def test_query_ingested_content(self):
    ...
    query_data = {"query": "HTML page", "limit": 5, "similarity_threshold": 0.1, "ranker_type": "weighted", "ranker_params": {"weights": [0.7, 0.3]}}
  ...
  def test_job_termination(self):
    ...
    ingest_data = {"url": test_url, "description": "Job for termination test", "default_chunk_size": 1000, "default_chunk_overlap": 100, "check_for_site_map": True, "sitemap_max_urls": 0}
  ...
  def test_error_scenarios(self):
    ...
    invalid_ingest_data = {"url": "not-a-valid-url", "description": "Invalid URL test", "default_chunk_size": 1000, "default_chunk_overlap": 100}
    ...
    response = requests.delete(f"{self.base_url}/v1/datasource/delete", params={"datasource_id": fake_datasource_id})
  ...
  def test_datasource_reload(self):
    ...
    response = requests.post(f"{self.base_url}/v1/datasource/reload", params={"datasource_id": test_datasource_id})
    ...
    assert job_data["status"] in ["completed", "completed_with_errors"], f"Reload job failed: {job_data.get('message', 'Unknown error')}"
  ...
  def test_datasource_cleanup(self):
    ...
    response = requests.delete(f"{self.base_url}/v1/datasource/delete", params={"datasource_id": test_datasource_id})
  ...
  @pytest.mark.skipif(os.getenv("ENABLE_GRAPH_RAG", "true").lower() != "true", reason="Graph RAG disabled")
  ...
  def test_graph_ingestor_cleanup(self):
    ...
    if hasattr(self, "test_ingestor_id"):
      response = requests.delete(f"{self.base_url}/v1/graph/ingestor/delete", params={"ingestor_id": self.test_ingestor_id})
