...
if not GITHUB_TOKEN and not (GITHUB_APP_ID and GITHUB_APP_PRIVATE_KEY and GITHUB_APP_INSTALLATION_ID):
  raise ValueError("Authentication required: Either set GITHUB_TOKEN (for PAT) or GITHUB_APP_ID + GITHUB_APP_PRIVATE_KEY + GITHUB_APP_INSTALLATION_ID (for GitHub App)")
...
async def sync_github_entities(client: Client):
  ...
  if GITHUB_TOKEN:
    ...
    github_app_auth = GitHubAppAuth(app_id=GITHUB_APP_ID or "", private_key=GITHUB_APP_PRIVATE_KEY or "", installation_id=GITHUB_APP_INSTALLATION_ID or "", rest_api_url=GITHUB_REST_API_URL)
...
if __name__ == "__main__":
  try:
    ...
    IngestorBuilder().name(github_instance_name).type("github").description("Ingestor for GitHub entities" + (f" from organization {GITHUB_ORG}" if GITHUB_ORG else "")).metadata(
      {"github_api_url": GITHUB_API_URL, "github_org": GITHUB_ORG, "sync_interval": SYNC_INTERVAL, "auth_method": auth_method}
    ).sync_with_fn(sync_github_entities).every(SYNC_INTERVAL).with_init_delay(int(os.getenv("INIT_DELAY_SECONDS", "0"))).run()
