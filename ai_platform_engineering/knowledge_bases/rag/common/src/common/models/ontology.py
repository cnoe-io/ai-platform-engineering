...
class FkeyHeuristic(BaseModel):
  ...
  entity_a_type: Annotated[str, WithJsonSchema({"prompt_exposed": "true"})] = Field(description="Source entity type (e.g., 'User')")
  entity_b_type: Annotated[str, WithJsonSchema({"prompt_exposed": "true"})] = Field(description="Target entity type (e.g., 'Account')")
  property_mappings: Annotated[List[PropertyMappingStructure], WithJsonSchema({"prompt_exposed": "true"})] = Field(description="Property mappings that define this relationship structure (which properties connect). Example: [{'entity_a_property': 'user_id', 'entity_b_idkey_property': 'id'}]")
  ...
  property_match_patterns: Annotated[dict[str, dict[str, int]], WithJsonSchema({"prompt_exposed": "true"})] = Field(
    default_factory=dict, description="Per-property match type distribution showing data quality. Key: 'entity_a_prop->entity_b_prop', Value: {match_type: count}. Example: {'user_id->id': {'exact': 45, 'prefix': 5}}"
  )
  property_match_quality: Annotated[dict[str, dict[str, float]], WithJsonSchema({"prompt_exposed": "true"})] = Field(
    default_factory=dict, description="Per-property per-match-type quality averages. Key: 'entity_a_prop->entity_b_prop', Value: {match_type: avg_quality}. Example: {'user_id->id': {'exact': 1.0, 'prefix': 0.76}}"
  )
  value_match_quality_avg: Annotated[float, WithJsonSchema({"prompt_exposed": "true"})] = Field(default=0.0, description="Average quality of value matches (0.0-1.0). Higher = more exact matches.")
  deep_match_quality_avg: Annotated[float, WithJsonSchema({"prompt_exposed": "true"})] = Field(default=0.0, description="Average deep match quality score considering BM25, value quality, uniqueness, and simplicity (0.0+). Higher = stronger relationship signal.")

  # Internal Fields - For incremental updates and processing
  value_match_quality_sum: float = Field(default=0.0, description="[Internal] Running sum of value match quality for calculating average")
  deep_match_quality_sum: float = Field(default=0.0, description="[Internal] Running sum of deep match quality for calculating average")
  last_processed: int = Field(default=0, description="[Internal] Timestamp of last heuristic update")
  ...
  def get_property_quality(self, entity_a_prop: str, entity_b_prop: str) -> float:
    ...
    weighted_sum = sum(patterns.get(match_type, 0) * quality for match_type, quality in qualities.items())
...
class FkeyEvaluation(BaseModel):
  ...
  property_mappings: List[PropertyMappingRule] = Field(description="The property mapping rules to use when applying this relationship. Specifies which properties to match and what matching strategy (match_type) to use.")
...
class EntityExample(BaseModel):
  ...
  properties: dict[str, Any] = Field(default_factory=dict, description="Properties of the entity (filtered to only include mapped properties for examples)")
  sub_entities: List["EntityExample"] = Field(default_factory=list, description="Sub-entities of this entity (recursive, max depth 10)")
...
class RelationCandidateGroup(BaseModel):
  ...
  candidates: List[RelationCandidate] = Field(description="List of relation candidates between these two entity types")
  existing_relations: List[str] = Field(default_factory=list, description="Names of relations already accepted between these two entity types")
...
class CandidateGroupData(BaseModel):
  ...
  examples: List[tuple[EntityExample, EntityExample]] = Field(default_factory=list, description="Up to 3 example entity pairs showing the relationship (with sub-entities loaded)")
  heuristic_change_reason: str = Field(default="", description="Explanation of why this group is being re-evaluated (e.g., 'count increased by 25%')")
