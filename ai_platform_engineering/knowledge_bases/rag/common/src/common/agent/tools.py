...
async def search(query: str, structured_entity_type: Optional[str] = "", datasource_id: Optional[str] = "", limit: int = 5, similarity_threshold: float = 0.3, thought: str = "") -> str:
  ...
  try:
    ...
    async with httpx.AsyncClient() as client:
      response = await client.post(server_url + "/v1/query", json=api_payload, timeout=30.0)
...
async def graph_check_if_ontology_generated(thought: str) -> str:
  ...
  try:
    ...
    relation = await ontology_graphdb.find_relations(None, None, None, {ONTOLOGY_VERSION_ID_KEY: ontology_version_id}, 1)
