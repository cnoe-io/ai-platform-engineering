...
import pytest
from agent_ontology.bm25_search_engine import tokenize_value, tokenize_pascal_case, tokenize_record, tokenize_query, BM25SearchEngine
...
class TestTokenizationFunctions:
  ...
  def test_tokenize_record_with_entity_type(self):
    ...
    result = tokenize_record(entity_pk="pk-123", entity_type="UserAccount", all_ids=["user-456", "test@email.com"])
  ...
  def test_tokenize_record_empty_ids(self):
    ...
    result = tokenize_record(entity_pk="pk-123", entity_type="User", all_ids=[])
    # Should still have entity type
    assert "user" in result

  def test_tokenize_record_deduplication(self):
    """Test record tokenization removes duplicates"""
    result = tokenize_record(entity_pk="test", entity_type="Test", all_ids=["test", "test"])
