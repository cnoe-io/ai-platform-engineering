...
class AgentWorker:
  ...
  def __init__(self, worker_id: int, groups_queue: list[CandidateGroupData], rc_manager: RelationCandidateManager, data_graph_db: GraphDB, agent_recursion_limit: int, progress_tracker: dict[str, int] | None = None):
    """
  ...
  async def fetch_next_relation_candidate(self, thought: str) -> str:
    ...
    if self.progress_tracker is not None:
      ...
      self.evaluation_logger.info(f"Worker {self.worker_id}: Loaded group {self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type} (Progress: {completed}/{total}, {len(self.groups_queue)} remaining in worker queue)")
    else:
      self.evaluation_logger.info(f"Worker {self.worker_id}: Loaded group {self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type} ({len(self.groups_queue)} groups remaining in worker queue)")
    ...
    response = {
      ...
      "examples": [
        {
          "entity_a": {"type": ex[0].entity_type, "primary_key": ex[0].primary_key, "properties": ex[0].properties, "sub_entities": [{"type": sub.entity_type, "primary_key": sub.primary_key, "properties": sub.properties} for sub in ex[0].sub_entities]},
          "entity_b": {"type": ex[1].entity_type, "primary_key": ex[1].primary_key, "properties": ex[1].properties, "sub_entities": [{"type": sub.entity_type, "primary_key": sub.primary_key, "properties": sub.properties} for sub in ex[1].sub_entities]},
        }
  ...
  async def query_existing_relations(self, entity_a_type: str, entity_b_type: str, thought: str) -> str:
    ...
    try:
      ...
      relations = await self.rc_manager.ontology_graph_db.find_relations(from_entity_type=entity_a_type, to_entity_type=entity_b_type, properties={constants.ONTOLOGY_VERSION_ID_KEY: self.rc_manager.ontology_version_id})
    ...
    async with self._accept_reject_lock:
      try:
        ...
        if candidate is None:
          self.evaluation_logger.error(f"Worker {self.worker_id}: Relation {relation_id} does NOT belong to current group ({self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type})")
          return f"Error: Relation {relation_id} does not belong to the current group ({self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type}). You can only accept relations from the group you most recently fetched."

        # Validate: Property mappings must match the candidate's heuristic structure
        heuristic_mappings = {(pm.entity_a_property, pm.entity_b_idkey_property) for pm in candidate.heuristic.property_mappings}
        provided_mappings = {(pm.entity_a_property, pm.entity_b_idkey_property) for pm in property_mappings}
        ...
        self.evaluation_logger.debug(f"Worker {self.worker_id}: Group ({self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type}) has {self.accepted_count} accepted relation(s) so far. Attempting to accept {relation_id} with name '{relation_name}'")

        # Warn if accepting multiple relations in same group
        if self.accepted_count >= 1:
          self.evaluation_logger.warning(
            f"Worker {self.worker_id}: Trying to accept multiple relations in group ({self.current_group_data.group.entity_a_type} -> {self.current_group_data.group.entity_b_type}). Already has {self.accepted_count} accepted relation(s). Relation: {relation_id}, Name: {relation_name}"
          )
          if "multiple" not in justification.lower() and "several" not in justification.lower():
            return f"Warning: This group already has {self.accepted_count} accepted relation(s). You should typically only accept ONE relation between entity types unless there's very strong evidence for multiple relations. If you're certain, include 'multiple' or 'several' in your justification."
        ...
        self.evaluation_logger.debug(f"Worker {self.worker_id}: Accepted relation {relation_id}. Total accepted in this group: {self.accepted_count}")
  ...
  async def reject_relation(self, relation_id: str, justification: str, thought: str) -> str:
    ...
    async with self._accept_reject_lock:
      try:
        ...
        property_mapping_rules = [PropertyMappingRule(entity_a_property=pm.entity_a_property, entity_b_idkey_property=pm.entity_b_idkey_property, match_type=ValueMatchType.NONE) for pm in heuristic.property_mappings]

        await self.rc_manager.update_evaluation(relation_id=relation_id, relation_name=constants.CANDIDATE_RELATION_NAME, result=FkeyEvaluationResult.REJECTED, justification=justification, thought=thought, is_manual=True, property_mappings=property_mapping_rules)
  ...
  async def mark_relation_unsure(self, relation_id: str, justification: str) -> str:
    ...
    async with self._accept_reject_lock:
      try:
        ...
        property_mapping_rules = [PropertyMappingRule(entity_a_property=pm.entity_a_property, entity_b_idkey_property=pm.entity_b_idkey_property, match_type=ValueMatchType.EXACT) for pm in heuristic.property_mappings]
  ...
  async def create_agent(self, system_prompt: str, base_llm) -> CompiledStateGraph:
    ...
    agent = create_react_agent(base_llm, tools=tools, prompt=system_prompt, state_schema=State, pre_model_hook=SummarizationNode(token_counter=count_tokens_approximately, model=base_llm, max_tokens=max_llm_tokens, max_summary_tokens=max_summary_tokens, output_messages_key="llm_input_messages"))
