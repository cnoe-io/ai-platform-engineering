...
class HeuristicsProcessor:
  ...
  def __init__(self, graph_db: GraphDB, rc_manager: RelationCandidateManager, entity_batch_size: int = 10_000, index_batch_size: int = 50_000, min_relation_count: int = 3, top_score_matches_only: bool = True, progress_callback: Callable[[str], Awaitable[None]] | None = None):
    ...
    # Statistics
    self.stats = {"entities_processed": 0, "relations_created": 0, "fuzzy_searches": 0, "fuzzy_searches_filtered": 0, "deep_matches": 0, "relations_discarded": 0}
  ...
  async def process_all_entities(self):
    ...
    self.search_engine = BM25SearchEngine(self.graph_db, index_batch_size=self.index_batch_size)
    ...
    while type_idx < len(entity_types):
      ...
      entities = await self.graph_db.fetch_entities_batch(offset=current_type_offset, limit=remaining_capacity, entity_type=current_type)
  ...
  async def _process_entities_pipeline(self, entities: List[StructuredEntity], batch_num: int, total_batches: int, update_progress):
    ...
    for entity in entities:
      ...
      if entity.additional_types and SUB_ENTITY_LABEL in entity.additional_types:
        ...
        if parent_entity_type:
          ...
          relation_id = self.rc_manager.generate_relation_id(entity.entity_type, parent_entity_type, property_mappings)
          if logger.isEnabledFor(logging.DEBUG):
      ...
      for property_name, prop_value_raw in entity.all_properties.items():
        ...
        for prop_value in prop_values:
          ...
          search_tasks.append(SearchTask(entity=entity, property_name=property_name, property_value=prop_value, query_tokens=query_tokens, entity_property_id_key=entity_property_id_key))
    ...
    for task, matches in zip(search_tasks, all_results):
      ...
      deep_matches = self.deep_property_match(task.entity, task.property_name, task.property_value, matches, logger)
      ...
      for dm in deep_matches:
        if not dm.matching_properties:
          logger.warning("No property mappings found for entity %s (%s) and matched entity %s (%s).", task.entity.entity_type, task_entity_pk, dm.matched_entity_type, dm.matched_entity_pk)
          continue

        relation_id = self.rc_manager.generate_relation_id(task.entity.entity_type, dm.matched_entity_type, dm.matching_properties)
      ...
      if logger.isEnabledFor(logging.DEBUG) and logged_matches:
        ...
        for i, dm in enumerate(logged_matches):
          ...
          for pm in dm.matching_properties:
            mappings_details.append(f"{pm.entity_a_property}←→{pm.entity_b_idkey_property}[{pm.match_type},q={pm.value_match_quality:.2f}]")
  ...
  def calculate_deep_match_quality(self, bm25_score: float, property_mappings: List[PropertyMapping], identity_key: dict[str, Any], num_valid_mappings: int) -> float:
    """
  ...
  def deep_property_match(self, entity: StructuredEntity, entity_property: str, entity_property_value: str, matches: List[Tuple[Dict[str, Any], float]], logger: logging.Logger) -> List[DeepPropertyMatch]:
    ...
    for idx, (matched_dict, score) in enumerate(matches):
      ...
      for idkey_idx, identity_key in enumerate(matched_entity_identity_keys):
        ...
        matched_entity_idkey_property = next((k for k, v in identity_key.items() if self.is_matching(entity_property_value, v).is_match), None)

        if not matched_entity_idkey_property:
          continue

        # Find matching properties
        entity_prop_dict = {k: entity.all_properties[k] for k in entity.all_properties if not k.startswith("_")}
        ...
        for _, (mapping_dict, match_metadata) in enumerate(match_prop_mappings):
          ...
          deep_match_quality = self.calculate_deep_match_quality(bm25_score=float(score), property_mappings=property_mappings, identity_key=identity_key, num_valid_mappings=len(match_prop_mappings))
  ...
  def find_matching_key_mappings(self, reference_dict: dict[str, Any], target_dict: dict[str, Any], must_have_mapping: dict[str, str], logger: logging.Logger, flexible_match_keys: set[str] | None = None) -> List[Tuple[dict[str, Any], dict[str, MatchResult]]]:
    """
