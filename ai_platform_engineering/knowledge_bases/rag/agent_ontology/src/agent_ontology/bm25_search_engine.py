...
class BM25SearchEngine:
  ...
  def __init__(self, graph_db: GraphDB, index_batch_size: int = 50_000, bloom_filter_size: int = 10_000_000, bloom_filter_error_rate: float = 0.01):
    ...
    # Statistics
    self.stats = {"total_entities_indexed": 0, "total_tokens": 0, "bloom_filter_size": 0, "entity_types_count": 0}
  ...
  async def search(self, query_string: str, exclude_entity_type: str | None = None, top_k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
    """
  ...
  async def search_batch(
    ...
    for exclude_type, query_group in queries_by_exclude_type.items():
      ...
      if self.logger.isEnabledFor(logging.DEBUG) and masked_indices_all:
        ...
        sample_indices = [masked_indices_all[0], masked_indices_all[len(masked_indices_all) // 4], masked_indices_all[len(masked_indices_all) // 2], masked_indices_all[3 * len(masked_indices_all) // 4], masked_indices_all[-1]][:sample_size]
