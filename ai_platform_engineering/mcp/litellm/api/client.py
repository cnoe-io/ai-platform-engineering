...
API_TOKEN = os.getenv("LITELLM_API_KEY") or os.getenv("LITELLM_TOKEN") or os.getenv("LITELLM_API_TOKEN")

# SSL verification configuration with fallback
verify_ssl_env = os.getenv("LITELLM_VERIFY_SSL", None)
if verify_ssl_env is None:
  verify_ssl_env = os.getenv("VERIFY_SSL", "true")
VERIFY_SSL = verify_ssl_env.lower() == "true"

CA_BUNDLE = os.getenv("LITELLM_CA_BUNDLE")

if not API_URL:
  raise ValueError("LITELLM_API_URL environment variable is not set.")
if not API_TOKEN:
  raise ValueError("LITELLM_API_KEY, LITELLM_TOKEN, or LITELLM_API_TOKEN environment variable is not set.")
...
async def make_api_request(
  ...
  if not token:
    ...
    return (
      False,
      {"error": "Token is required. Please set LITELLM_API_KEY, LITELLM_TOKEN, or LITELLM_API_TOKEN."},
    )
