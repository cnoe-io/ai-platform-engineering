...
Also exposes a thin reachability probe (``GET /agents/{agent_id}/probe``)
used by the autonomous-agents service to verify that a configured
``dynamic_agent_id`` actually points at a real agent before it tries to
schedule executions through it. The probe is intentionally tiny -- it
returns just enough metadata for the autonomous-agents UI to render an
"Ack OK" badge with the agent's display name, and nothing else (no system
prompts, tool allowlists, or other internals that could leak to a
less-privileged service-to-service caller).
"""
