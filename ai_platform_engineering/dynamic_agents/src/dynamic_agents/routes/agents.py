...
async def probe_agent(
    ...
    ``dynamic_agent_id`` actually points at a real agent before it tries
    to schedule executions through it. Returns just enough metadata for
    the autonomous-agents UI to render an "Ack OK" badge with the
    ...
    ``X-User-Context`` header is gateway-trusted via ``get_current_user``.
