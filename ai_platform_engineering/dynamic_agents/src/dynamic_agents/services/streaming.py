...
class StreamingMixin:
    ...
    def _build_stream_config(self, session_id: str, user_id: str, trace_id: str | None) -> dict[str, Any]:
        ...
        # Spec 104: forward the user identity (and OBO bearer if any) into
        # AgentContext so downstream tools — most importantly the MCP httpx
        # factory — can sign their outbound calls to AGW with the user's
        # JWT instead of the runtime's service token. `user_name` and
        # `user_groups` are advisory metadata used by tools that need
        # display/auditing context.
