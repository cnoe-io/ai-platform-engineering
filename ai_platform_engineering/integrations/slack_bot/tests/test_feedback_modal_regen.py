...
def test_missing_block_does_not_regenerate():
  """No regen block present (e.g. malformed payload) defaults to no regen."""
  assert regenerate_requested({"correction_input": {}}) is False


def test_non_dict_input_does_not_regenerate():
  """Defensive: non-dict input must not raise."""
  assert regenerate_requested(None) is False
