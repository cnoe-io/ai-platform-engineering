...
class TestOverthinkIntegration:
  ...
  These tests verify that the agent returns the expected markers for different message types.
  """
  ...
  def test_overthink_returns_defer_for_mr_approval_request(self, mocker):
    ...
    This calls the real dynamic agents API with a human action request.
    """
    ...
    print("\n" + "=" * 80)
    print("Response for MR approval request (expecting [DEFER]):")
    print("=" * 80)
    print(final_text)
    print("=" * 80 + "\n")

    assert final_text is not None and final_text != "", "Should receive a response"
  ...
  def test_overthink_returns_low_confidence_for_obscure_question(self, mocker):
    ...
    This calls the real dynamic agents API with an obscure technical question.
    """
    ...
    print("\n" + "=" * 80)
    print("Response for obscure question (expecting [LOW_CONFIDENCE]):")
    print(final_text)

    assert final_text is not None and final_text != "", "Should receive a response"
    assert "[LOW_CONFIDENCE]" in final_text, f"Expected [LOW_CONFIDENCE] in response but got: {final_text[:200]}"
  ...
  def test_overthink_returns_high_confidence_for_documented_question(self, mocker):
    ...
    This calls the real dynamic agents API with a question that has documented answers.
    """
    ...
    print("\n" + "=" * 80)
    print("Response for documented question (expecting [CONFIDENCE: HIGH]):")
    print(final_text)
    ...
    assert "[CONFIDENCE: HIGH]" in final_text, f"Expected [CONFIDENCE: HIGH] marker in response but got: {final_text[:200]}"
...
class TestAIAlertProcessing:
  ...
  def test_informational_alert_no_ticket(self, mocker):
    ...
    dynamic_agents_url = os.getenv("DYNAMIC_AGENTS_URL", "http://localhost:8001")
    sse_client = SSEClient(dynamic_agents_url, timeout=60)
    ...
    responded = mock_slack_client.chat_startStream.called or mock_slack_client.chat_postMessage.called
  ...
  def test_alert_analysis_format(self, mocker):
    ...
    responded = mock_slack_client.chat_startStream.called or mock_slack_client.chat_postMessage.called or mock_slack_client.chat_update.called
    assert responded, "Should deliver a response to Slack"
