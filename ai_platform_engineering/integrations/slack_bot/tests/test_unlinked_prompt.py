...
"""Smoke tests for the unlinked-account fallback prompt.
...
The pure fallback module owns prompt construction, so content assertions do
not require the Bolt request boundary.
"""
...
def test_dead_end_message_is_no_longer_the_default() -> None:
    ...
    account") MUST NOT appear in the fallback prompt."""
...
def test_unlinked_prompt_offers_actionable_link() -> None:
    ...
    The fallback owns this stable user-facing copy.
    """
    src = _ANON_FALLBACK_PY.read_text(encoding="utf-8")
