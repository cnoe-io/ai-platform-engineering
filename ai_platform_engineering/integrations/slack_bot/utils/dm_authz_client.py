...
on direct user→agent grants and then traverses the user's team union
(Phase 1 PDP helper).

Design constraints (FR-007 / FR-009 / fail-closed):

- The bot uses stdlib ``urllib`` only — no new dependency.
- Any transport failure (no base URL, missing token, network, 5xx,
  malformed JSON) returns ``available=False`` which callers MUST treat
  as **deny + temporary-unavailable** rather than allow.
- ``allowed=False`` with a reason code is a legitimate deny — the bot
  surfaces a "you don't have access" message and does not retry.
"""
...
class DmAgentAccessDecision:
    """Outcome of a single BFF PDP probe.

    ``available`` distinguishes a real PDP decision (allow or deny) from
    a transport / infrastructure failure. Callers MUST fail-closed when
    ``available is False``.
    """
...
class DmAuthzClient:
    ...
    def check_agent_access(
        ...
        """Probe the BFF PDP for `user:<sub> can_use agent:<agent_id>`.

        Returns ``DmAgentAccessDecision(available=False)`` for any
        failure mode. Callers translate that to a "service temporarily
        unavailable" deny and never to an allow.
        """
        ...
        try:
            with self._open(request, timeout=self._timeout) as response:  # noqa: S310
                ...
                if status < 200 or status >= 300:
                    logger.info(
                        "dm_authz BFF non-2xx (status=%s)", status
                    )
                    return _UNAVAILABLE
                raw = response.read()
