...
class AIPlatformEngineerA2AExecutor(AgentExecutor):
    ...
    async def _handle_streaming_chunk(self, event: dict, state: StreamState,
        ...
        if is_tool_notification:
            ...
            # Prefer explicit source_agent; otherwise use tool name so clients see
            # github/argocd/write_todos (not a generic "supervisor" or "task").
            if event.get('source_agent'):
                source_agent = event['source_agent']
            elif 'tool_call' in event:
                source_agent = event['tool_call'].get('name') or 'supervisor'
            elif 'tool_result' in event:
                source_agent = event['tool_result'].get('name') or 'supervisor'
            else:
