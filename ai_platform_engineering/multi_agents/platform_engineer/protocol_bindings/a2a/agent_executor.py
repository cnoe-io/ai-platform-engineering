        ...
        if isinstance(metadata, dict) and metadata.get("resume"):
            ...
            for decision in decisions:
                ...
                if decision_type in ("accept", "approve"):
                    responses.append({"type": "accept", "action_name": action_name})
                elif decision_type == "edit":
                    edited_args = decision.get("args", {}).get("args", {})
                    responses.append({
                        "type": "edit",
                        "action_name": action_name,
                        "args": {"action": action_name, "args": edited_args}
                    })
            ...
            logger.info(f"📦 Created resume Command with {len(responses)} responses")
            resume_cmd = Command(
                resume=responses,
