...
def main():
  ...
  @mcp.custom_route(
    "/assets/finops-agent-workflow.svg",
    methods=["GET"],
    include_in_schema=False,
  )
  async def finops_agent_workflow(_request):
    return Response(
      usage_reports.FINOPS_WORKFLOW_COMPACT_SVG,
  ...
  @mcp.custom_route(
    "/assets/finops-agent-workflow-steps.svg",
    methods=["GET"],
    include_in_schema=False,
  )
  async def finops_agent_workflow_steps(_request):
    return Response(
      usage_reports.FINOPS_WORKFLOW_STEPS_SVG,
      media_type="image/svg+xml",
      headers={
        "Cache-Control": "public, max-age=300",
        "Access-Control-Allow-Origin": "*",
  ...
  mcp.tool()(usage_reports.get_litellm_report_request_form)
