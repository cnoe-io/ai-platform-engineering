...
"""Istio Agent implementation using common A2A base classes."""
...
_prompt_config = load_subagent_prompt_config("istio")


class IstioAgent(BaseLangGraphAgent):
    """Istio Agent for managing Istio service mesh resources."""
    ...
    def get_mcp_config(self, server_path: str) -> dict:
        """Return MCP configuration for Istio."""
    ...
    async def stream(self, query: str, sessionId: str, trace_id: str = None):
        ...
        try:
            ...
            logger.error(f"Unexpected Istio agent error: {str(e)}", exc_info=True)
            yield {
                'is_task_complete': True,
                'require_user_input': False,
                'kind': 'error',
                'content': f"An unexpected error occurred in Istio: {str(e)}",
            }
