...
"""SigNoz Agent implementation using common A2A base classes."""
...
_prompt_config = load_subagent_prompt_config("signoz")


class SigNozAgent(BaseLangGraphAgent):
    """SigNoz Agent for managing observability platform resources."""
    ...
    @trace_agent_stream("signoz")
    async def stream(self, query: str, sessionId: str, trace_id: str = None):
        """
        Stream responses with signoz-specific tracing and safety-net error handling.
        ...
        try:
            ...
            logger.error(f"Unexpected SigNoz agent error: {str(e)}", exc_info=True)
            yield {
                'is_task_complete': True,
                'require_user_input': False,
                'kind': 'error',
                'content': f"❌ An unexpected error occurred in SigNoz: {str(e)}\n\nPlease try again or contact support if the issue persists.",
            }
