...
class ResponseFormat(BaseModel):
    """Response format for the VictorOps agent."""
    status: Literal['input_required', 'completed', 'error'] = 'input_required'
    message: str

class VictorOpsAgent:
    """VictorOps Agent."""

    SYSTEM_INSTRUCTION = """You are a helpful assistant that can interact with VictorOps.
    You can use the VictorOps API to get information about incidents, services, and schedules.
    ...
    def __init__(self):
        logger.info("Initializing VictorOpsAgent")
    ...
    async def initialize(self):
        ...
        server_path = "./agent_victorops/protocol_bindings/mcp_server/mcp_victorops_mcpapi/server.py" #victorops_mcp/ does not exist 
        print(f"Launching MCP server at: {server_path}")

        victorops_api_url = os.getenv("VICTOROPS_API_URL")
        if not victorops_api_url:
            logger.error("VICTOROPS_API_URL not set in environment")
        ...
        async for item in self.graph.astream(inputs, config, stream_mode='values'):
            ...
            if (
                ...
                yield {
                    ...
                    'content': 'Looking up VictorOps information...',
                }
                ...
                yield {
                    'is_task_complete': False,
                    'require_user_input': False,
                    'content': 'Processing VictorOps data...',
                }
