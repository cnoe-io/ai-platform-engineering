...
export function buildMcpRemoteOAuthArgs({ endpoint, allowHttp }: McpRemoteOAuthArgsOptions): string[] {
  // mcp-remote follows the MCP OAuth discovery chain and dynamically
  // registers its own public PKCE client. Do not couple the bundle to a
  // deployment-specific, pre-registered Keycloak client id.
