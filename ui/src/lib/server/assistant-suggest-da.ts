    ...
    if (isLocal && SUSPICIOUS_LOCAL_PORTS.has(u.port)) {
      console.warn(
        `[dynamic-agents] DYNAMIC_AGENTS_URL=${base} looks suspicious. ` +
          `The dynamic-agents host port is typically 8200 (compose: 8200->8001). ` +
...
export function getDynamicAgentsSuggestUrl(): string {
  const base = process.env.DYNAMIC_AGENTS_URL || "http://localhost:8200";
...
export function formatAssistantSuggestFetchError(
  ...
  if (code === "ECONNREFUSED" || code === "ENOTFOUND") {
    return (
      ...
      `The UI server reads DYNAMIC_AGENTS_URL (e.g. ui/.env.local); default is http://localhost:8200. ` +
