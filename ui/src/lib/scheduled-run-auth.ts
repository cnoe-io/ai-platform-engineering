 ...
 *
 *   1. loads the immutable owner from the schedule DB record (never from a
 *      runner-supplied request field),
 ...
 *
 * This mirrors the proven Python OBO impersonation used by the Slack/Webex
 * bots (integrations/{slack,webex}_bot/utils/obo_exchange.py impersonate_subject).
 */
 ...
 * absent or no secret is configured, so the caller treats the request as a
 * normal interactive call rather than a scheduled run.
 */
...
interface ScheduleOwnerDoc {
  schedule_id: string;
  owner_user_id?: string;
  /**
   * Optional pre-resolved Keycloak ``sub`` of the owner. Preferred over
   * ``owner_user_id`` (email) when present because it is stable across email
   * renames. Nothing populates it today; the email path below covers every
   * existing schedule and this is the forward-compatible fast path.
   */
  owner_sub?: string;
}

export interface ScheduledRunOwner {
...
/**
 * Resolve the authoritative owner of a schedule from the DB record, then map it
 * to a Keycloak ``sub``.
 *
 * Owner identity comes ONLY from the persisted schedule (FR: "owner is resolved
 * only from the schedule DB record"), never from a runner-supplied request
 * field. Returns ``null`` when the schedule or the owner's Keycloak account
 * cannot be found, so the caller fails the run closed.
 */
export async function resolveScheduledRunOwner(
  scheduleId: string,
): Promise<ScheduledRunOwner | null> {
  if (!scheduleId || !isMongoDBConfigured) return null;

  const schedules = await getCollection<ScheduleOwnerDoc>("schedules");
  ...
  if (storedSub) {
    return { sub: storedSub, email };
  }

  if (!email) {
    console.error(
      `[scheduled-run-auth] schedule ${scheduleId} has no owner_user_id/owner_sub`,
    );
    return null;
  }
