/**
 * Pure stage resolver for Agentic SDLC.
 *
 * `resolveStage(input) → AgenticSdlcStage`
 *
...
export interface ResolveStageInput {
  ...
  labelOverrides?: Record<string, AgenticSdlcStage>;
...
/** Pure resolver. */
export function resolveStage(input: ResolveStageInput): AgenticSdlcStage {
  ...
  const merged: Record<string, AgenticSdlcStage> = {
    ...DEFAULT_AGENT_LABEL_TO_STAGE,
  ...
  // in review.
  const labelStagePriority: AgenticSdlcStage[] = [
    "blocked",
    "observe",
    "validate",
    "deploy",
  ...
  const labeledStages = new Set<AgenticSdlcStage>();
