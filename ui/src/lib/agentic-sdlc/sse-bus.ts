/**
 * In-process SSE fanout bus for the Agentic SDLC.
 *
 ...
 *
 * Topics:
 *   - `portfolio`                  all accessible repo activity
 *   - `epic:<repo_id>:<epic_id>`   per-Epic stream
 *   - `repo:<repo_id>`             per-repo stream for detail pages
...
export function repoTopic(repoId: string): string {
  return `repo:${repoId}`;
}

export function portfolioTopic(): string {
  return "portfolio";
}
