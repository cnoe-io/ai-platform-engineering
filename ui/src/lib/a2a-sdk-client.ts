...
export interface HITLDecision {
  /** Decision type: approve, edit, reject, or response */
  type: 'approve' | 'accept' | 'edit' | 'reject' | 'response';
  /** Action/tool name this decision applies to */
  actionName: string;
  /** Edited args (for 'edit' type) or form values (for 'approve') */
  args?: Record<string, unknown>;
  /** Message/reason (for 'reject' or 'response' type) */
  message?: string;
}
...
export class A2ASDKClient {
  ...
  /**
   * HITL Decision types for form responses
   */
  static readonly DECISION_TYPES = {
    APPROVE: 'approve',
    ACCEPT: 'accept',  // Alias for approve
    EDIT: 'edit',
    REJECT: 'reject',
    RESPONSE: 'response',  // For free-form text responses
  } as const;
