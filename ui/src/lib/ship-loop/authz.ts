/**
 * Repo-scoped authorization helpers for the Ship Loop.
 *
 * Per spec FR-024: Ship Loop relies *exclusively* on the caller's
 * GitHub permissions for HITL actions; there is no internal Ship-Loop
 * role. Concretely:
 *
 *   - any authenticated user with `pull` ⇒ can READ a repo's Ship Loop
 *     dashboard.
...
import {
  ...
  createGitHubClient,
  type IGitHubClient,
} from "@/lib/ship-loop/github-client";

export type ShipLoopPermissionLevel =
  | "none"
  | "read"
  | "comment"
  | "admin";
...
export async function resolveRepoPermission(
  opts: ResolveOpts,
): Promise<ShipLoopPermissionLevel> {
  ...
  let level: ShipLoopPermissionLevel = "none";
  try {
...
export class ShipLoopAuthzError extends Error {
  readonly status: 401 | 403 | 404;
  constructor(message: string, status: 401 | 403 | 404) {
    super(message);
    this.name = "ShipLoopAuthzError";
    this.status = status;
  }
}

/**
 * Throws `ShipLoopAuthzError` if the level isn't sufficient. Returns
...
export async function requireRepoPermission(
  opts: ResolveOpts,
  required: Exclude<ShipLoopPermissionLevel, "none">,
): Promise<ShipLoopPermissionLevel> {
  const level = await resolveRepoPermission(opts);
  if (rank(level) >= rank(required)) return level;
  if (level === "none") {
    throw new ShipLoopAuthzError("Repo not found or not accessible", 404);
  }
  throw new ShipLoopAuthzError(
    `Required GitHub permission '${required}' not granted; have '${level}'.`,
    403,
  );
}

function rank(level: ShipLoopPermissionLevel): number {
  switch (level) {
    case "admin":
      return 3;
    case "comment":
