...
| `assistant-guard.ts` | TBD | rejects mutating tool calls from the AG-UI assistant (lands with US5). |

## Import rules

**None** of these modules are safe to import from client components. The
client side talks to them only through `/api/ship-loop/**` routes and
the SSE channels. The two-layer toggle is enforced both:

1. server-side, via `guard.ts#isShipLoopServerEnabled` and
   `withShipLoopGate` (returns `404` when off), and
