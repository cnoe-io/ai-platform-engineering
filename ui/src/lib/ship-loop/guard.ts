/**
 * Server-side toggle gate for Ship Loop API routes.
 *
...
export type ShipLoopRouteHandler = (req: any, ctx?: any) => Promise<Response> | Response;

const NOT_FOUND_RESPONSE = (): Response =>
  new Response(null, { status: 404 });

/**
 * Wrap a Ship Loop route handler with the server-side toggle check.
 *
 ...
 */
export function withShipLoopGate(
  handler: ShipLoopRouteHandler,
): ShipLoopRouteHandler {
 ...
 * before rendering the Ship Loop tree. Caller should follow up with
 * Next's `notFound()` helper when this returns false.
 */
export function isShipLoopServerEnabled(): boolean {
