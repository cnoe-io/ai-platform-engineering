/**
 * In-process async worker for Ship Loop event projection.
 *
 ...
 *   - Project to a `ShipLoopArtifact` upsert + (in some cases) emit a
 *     downstream `ShipLoopEvent` row (e.g. translating a label to a
 *     stage_transition).
...
import { recordProjectionLatency, setWorkerQueueDepth } from "@/lib/ship-loop/sli";
import { epicTopic, publish } from "@/lib/ship-loop/sse-bus";
import {
  getShipLoopArtifactsCollection as shipLoopArtifacts,
...
export function shipLoopWorkerQueueDepth(): number {
  return queue.length;
}

export function enqueueShipLoopEvent(
  eventId: ObjectId,
  ...
  try {
    while (queue.length > 0) {
      ...
      try {
        ...
        try {
          const events = await shipLoopEvents();
 ...
 */
export async function shipLoopWorkerProcessOne(): Promise<boolean> {
  const item = queue.shift();
 ...
 */
export async function shipLoopWorkerDrain(): Promise<number> {
  let n = 0;
  while (await shipLoopWorkerProcessOne()) n++;
  return n;
}
...
async function processOneById(
  ...
  const ev = evDoc as ShipLoopEvent;

  const repos = await shipLoopRepos();
  ...
  if (artifactPatch) {
    const artifacts = await shipLoopArtifacts();
    const now = new Date();
    const filter = {
      repo_id: ev.repo_id,
    ...
    const final = (await artifacts.findOne(filter)) as ShipLoopArtifact | null;
    if (final && ev.epic_id) {
      const topic = epicTopic(ev.repo_id, ev.epic_id);
      publish(topic, {
...
function sanitizeArtifactForSse(a: ShipLoopArtifact): Record<string, unknown> {
  const { _id, ...rest } = a;
  void _id;
  return rest;
}

function sanitizeEventForSse(e: ShipLoopEvent): Record<string, unknown> {
