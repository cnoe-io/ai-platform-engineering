 ...
 */
export async function collectForwardedCredentials(
  sub: string,
  providers: string[],
  ...
  for (const provider of providers) {
    ...
    if (provider === "atlassian") {
      try {
        ...
        if (r.ok) {
          const sites = (await r.json().catch(() => [])) as Array<{
            id?: string;
            url?: string;
            scopes?: string[];
          }>;
          // accessible-resources can list separate entries per product (Jira,
          // Confluence). Prefer the Confluence-scoped one so the cloud_id is
          // valid for the Confluence API; fall back to the first site.
          const site =
            sites.find((s) => (s.scopes ?? []).some((sc) => sc.includes("confluence"))) ??
            sites[0];
          if (site?.id) entry.cloud_id = site.id;
          if (site?.url) entry.site_url = site.url;
        }
      } catch {
