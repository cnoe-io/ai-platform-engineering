...
import type { PlanStep, A2AEvent } from "@/types/a2a";
...
// ─── buildFromEvents — AG-UI wrapped A2AEvent format ────────────────────────

describe("TimelineManager.buildFromEvents (AG-UI wrapped events)", () => {
  /** Create a minimal A2AEvent skeleton */
  ...
  it("parses tool_start/tool_end A2AEvents with plain-name description (AG-UI format)", () => {
    // The chat store creates tool_start events with description=toolName (no "Tool call started:" prefix)
    const events: A2AEvent[] = [
      makeA2AEvent({
        ...
        artifact: {
          ...
          description: "search_confluence",
          parts: [{ kind: "text", text: "search_confluence" }],
          metadata: { sourceAgent: "search_confluence" },
        },
      }),
      makeA2AEvent({
        id: "evt-2",
    ...
    const segments = TimelineManager.buildFromEvents(events);
    expect(segments).toHaveLength(1);
    expect(segments[0].type).toBe("tool_call");
    expect(segments[0].toolCall?.tool).toBe("search_confluence");
    expect(segments[0].toolCall?.status).toBe("completed");
  });

  it("parses execution_plan_update A2AEvents with JSON text part (AG-UI format)", () => {
    // The chat store creates plan events with a text part containing JSON.stringify({ steps })
    const steps = [
      { id: "s1", agent: "RAG", description: "Search docs", status: "in_progress" },
      { id: "s2", agent: "RAG", description: "Synthesize", status: "pending" },
    ];
    const events: A2AEvent[] = [
      makeA2AEvent({
        type: "artifact",
        artifact: {
          ...
          parts: [{ kind: "text", text: JSON.stringify({ steps }) }],
        },
      }),
    ];
    ...
    expect(segments[0].planSteps![0]).toMatchObject({ id: "s1", description: "Search docs", status: "in_progress" });
    expect(segments[0].planSteps![1]).toMatchObject({ id: "s2", status: "pending" });
  });

  it("handles a full AG-UI wrapped sequence: plan -> tool -> final answer", () => {
    const steps = [
    ...
    const events: A2AEvent[] = [
      ...
      makeA2AEvent({
        ...
        artifact: {
          artifactId: "tc-1",
          name: "tool_notification_start",
          description: "search_docs",
          parts: [{ kind: "text", text: "search_docs" }],
          metadata: { sourceAgent: "search_docs" },
        },
      }),
      makeA2AEvent({
        id: "evt-3",
      ...
      makeA2AEvent({
        id: "evt-4",
        type: "artifact",
        displayContent: "Here is the answer.",
        artifact: {
          artifactId: "final-1",
          name: "final_result",
    ...
    const tool = segments.find((s) => s.type === "tool_call");
    expect(tool?.toolCall?.tool).toBe("search_docs");
  ...
  it("preserves backward compatibility with legacy A2A DataPart plan events", () => {
    // Legacy format: data part with step_id/title fields
    const events: A2AEvent[] = [
      makeA2AEvent({
        type: "execution_plan",
        artifact: {
          ...
          parts: [
            {
              ...
              data: {
                steps: [
                  { step_id: "s1", title: "Search SCS docs", agent: "RAG", status: "in_progress", order: 0 },
                  { step_id: "s2", title: "Synthesize", agent: "RAG", status: "pending", order: 1 },
                ],
              },
            },
          ],
    ...
    expect(segments[0].type).toBe("execution_plan");
    expect(segments[0].planSteps).toHaveLength(2);
    expect(segments[0].planSteps![0]).toMatchObject({ id: "s1", description: "Search SCS docs", status: "in_progress" });
  });

  it("ignores malformed JSON in text part plan events", () => {
    const events: A2AEvent[] = [
      makeA2AEvent({
        type: "artifact",
        artifact: {
          artifactId: "plan-bad",
          name: "execution_plan_update",
          parts: [{ kind: "text", text: "not valid json {{{" }],
        },
      }),
    ];

    // Should not throw, should produce no segments
    const segments = TimelineManager.buildFromEvents(events);
