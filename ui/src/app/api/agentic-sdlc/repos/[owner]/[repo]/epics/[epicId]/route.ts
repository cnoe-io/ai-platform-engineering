...
import type { AgenticSdlcArtifact, AgenticSdlcEvent } from "@/types/agentic-sdlc";
...
type SafeEvent = Omit<AgenticSdlcEvent, "payload" | "_id">;

function stripPayload(ev: AgenticSdlcEvent): SafeEvent {
  ...
  const { _id: _ignoredId, payload: _ignoredPayload, ...rest } = ev as AgenticSdlcEvent & {
    _id?: unknown;
  };
  void _ignoredId;
...
async function handle(
  ...
  const subtasks: AgenticSdlcArtifact[] = [];
  const pull_requests: AgenticSdlcArtifact[] = [];
  const deploys: AgenticSdlcArtifact[] = [];
