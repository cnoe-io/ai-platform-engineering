/**
 * API proxy route for removing a sandbox policy rule.
 *
 * DELETE /api/dynamic-agents/sandbox/policy/[agentId]/rule/[ruleId]
 */
...
export async function DELETE(
  request: NextRequest,
  context: { params: Promise<{ agentId: string; ruleId: string }> }
): Promise<Response> {
  const { agentId, ruleId } = await context.params;
  ...
  const backendUrl = `${dynamicAgentsUrl}/api/v1/sandbox/policy/${agentId}/rule/${ruleId}`;
  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      console.error(
        `[sandbox/rule] Backend error: ${backendResponse.status}`,
        errorText
      );
      return NextResponse.json(
