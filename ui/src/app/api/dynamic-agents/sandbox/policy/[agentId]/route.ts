/**
 * API proxy route for sandbox policy management.
 *
 * GET  /api/dynamic-agents/sandbox/policy/[agentId] - Get current policy
 * PUT  /api/dynamic-agents/sandbox/policy/[agentId] - Update policy with hot reload
 */
...
async function proxyToBackend(
  ...
  const backendUrl = `${dynamicAgentsUrl}/api/v1/sandbox/policy/${agentId}`;
  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      console.error(
        `[sandbox/policy] Backend error: ${backendResponse.status}`,
        errorText
      );
      return NextResponse.json(
...
export async function GET(
  ...
  return proxyToBackend(request, agentId, "GET");
}
...
export async function PUT(
  ...
  return proxyToBackend(request, agentId, "PUT", body);
}
