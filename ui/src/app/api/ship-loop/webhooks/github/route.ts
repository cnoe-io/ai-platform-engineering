/**
 * GitHub webhook receiver for the Ship Loop.
 *
 * Synchronous part:
...
import { recordWebhook } from "@/lib/ship-loop/sli";
import {
  hashWebhookSecret,
  verifyGitHubWebhook,
} from "@/lib/ship-loop/webhook-verify";
import type {
  ActorKind,
  ArtifactKind,
  ShipLoopEvent,
...
export async function POST(req: Request): Promise<Response> {
  ...
  const reposCol = await getShipLoopReposCollection();
  ...
  enqueueShipLoopEvent(insertResult.insertedId);
  recordWebhook("accepted");
