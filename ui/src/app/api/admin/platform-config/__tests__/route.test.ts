...
describe("admin platform-config route", () => {
  ...
  it("returns the saved navigation configuration", async () => {
    mockGetCollection.mockResolvedValue({
      findOne: jest.fn().mockResolvedValue({
        _id: "platform_settings",
        top_nav: {
          order: ["home", "app-agentic-sdlc", "home"],
          hidden: ["app-agentic-sdlc", "app-agentic-sdlc"],
        },
      }),
      updateOne: jest.fn(),
    });
    ...
    expect(body.data.top_nav).toEqual({
      order: ["home", "app-agentic-sdlc"],
      hidden: ["app-agentic-sdlc"],
    });
  });

  it("normalizes and persists navigation configuration", async () => {
    ...
    const response = await PATCH(
      request("/api/admin/platform-config", {
        ...
        body: JSON.stringify({
          top_nav: {
            order: ["home", "app-agentic-sdlc", "home", null],
            hidden: ["app-agentic-sdlc", "app-agentic-sdlc", 42],
          },
        }),
      }),
    );

    expect((await response.json()).data.top_nav).toEqual({
      order: ["home", "app-agentic-sdlc"],
      hidden: ["app-agentic-sdlc"],
    });
