...
function makeMetricsCollection(
  orphans: WebexMetricsDoc[],
  metricsByWebexId: Record<string, WebexMetricsDoc>,
) {
  return {
    find: () => ({
      limit: () => ({
    ...
    findOne: async ({ webex_user_id }: { webex_user_id: string }) =>
      metricsByWebexId[webex_user_id] ?? null,
  };
}
...
function mockCollections(options: {
  ...
  const metricsColl = makeMetricsCollection(
    options.orphans ?? [],
    options.metricsByWebexId ?? {},
  );
  const teamsColl = makeTeamsCollection(options.teamsByEmail ?? {});
  mockGetCollection.mockImplementation(async (name: string) => {
    if (name === "webex_user_metrics") return metricsColl;
...
function makeRequest(query = ""): NextRequest {
  return new NextRequest(
    `http://localhost:3000/api/admin/webex/users${query ? `?${query}` : ""}`,
...
describe("GET /api/admin/webex/users", () => {
  ...
  it("includes an unlinked orphan row from metrics only when status is all or unlinked", async () => {
    mockCollections({
      orphans: [{ webex_user_id: "W-orphan" }],
    });
    ...
    expect(all.data.items).toEqual([
      expect.objectContaining({ webex_user_id: "W-orphan", link_status: "unlinked" }),
    ]);
  ...
  it("filters linked and unlinked rows by the status query param", async () => {
    const user: RealmUser = {
      ...
      enabled: true,
      attributes: { webex_user_id: ["W2"] },
    };
    mockListRealmUsersPage.mockResolvedValueOnce([user]);
    mockCollections({ orphans: [{ webex_user_id: "W-orphan" }] });

    const { GET } = await import("../route");

    const linked = await (await GET(makeRequest("status=linked"))).json();
    expect(linked.data.items).toEqual([
      expect.objectContaining({ webex_user_id: "W2", link_status: "linked" }),
    ]);
  ...
  it("paginates results using page and page_size", async () => {
    const users: RealmUser[] = ["W1", "W2", "W3"].map((wid, idx) => ({
      id: `kc-${idx}`,
      username: `user${idx}`,
      email: `user${idx}@example.com`,
      enabled: true,
      attributes: { webex_user_id: [wid] },
    }));
    ...
    expect(body.data.items[0]).toMatchObject({ webex_user_id: "W2" });
