 ...
 * underneath (the "loop" part of Agentic SDLC delivery loop). Agent tokens flow along
...
import { usePrefersReducedMotion } from "@/components/agentic-sdlc/visualizations/use-prefers-reduced-motion";

interface AgenticSdlcAnimationProps {
  className?: string;
  /**
...
export function AgenticSdlcAnimation({
  className,
  showAgents = true,
}: AgenticSdlcAnimationProps) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Animated diagram of the Agentic SDLC delivery loop: a horizontal pipeline of stages from Specify to Observe, with a feedback arc looping back from Observe to Specify"
    >
      <svg
      ...
      >
        <defs>
          {/* Track gradient — product palette (teal -> purple -> magenta). */}
