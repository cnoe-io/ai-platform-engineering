...
interface KanbanLaneSpec {
  ...
  title: string;
  badgeStage: ShipLoopStage;
  acceptStages: ShipLoopStage[];
}
...
export function KanbanView({
  ...
  return (
    <div
      className={cn(
        "grid w-full gap-3 grid-cols-1 md:grid-cols-3",
        className,
      )}
      role="list"
      aria-label="Ship loop kanban"
    >
      {LANES.map((lane) => {
        ...
        return (
          <section
            ...
            className={cn(
              "flex min-h-[14rem] flex-col gap-2 rounded-lg border p-3",
              visual.bgClass,
              visual.borderClass,
            )}
          >
