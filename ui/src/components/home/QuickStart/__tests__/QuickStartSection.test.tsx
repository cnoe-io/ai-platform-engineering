...
import { getQuickStartTabs, QUICK_START_TABS } from '../quickStartCards'
...
describe('QuickStartSection', () => {
  ...
  it('adds the TOME card to Explore only when TOME is enabled', () => {
    const exploreWithTome = getQuickStartTabs({ tomeEnabled: true }).find(
      (tab) => tab.id === 'explore',
    )
    const exploreWithoutTome = getQuickStartTabs({ tomeEnabled: false }).find(
      (tab) => tab.id === 'explore',
    )

    expect(exploreWithTome?.cards.map((card) => card.id)).toEqual([
      'chat-assistants',
      'explore-tome',
    ])
    expect(exploreWithoutTome?.cards.map((card) => card.id)).toEqual([
      'chat-assistants',
    ])
  })
