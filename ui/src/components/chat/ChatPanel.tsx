...
export function ChatPanel({ endpoint }: ChatPanelProps) {
  ...
  const submitMessage = useCallback(async (messageToSend: string) => {
    ...
    // treat as a "response" type rejection with their message
    // ═══════════════════════════════════════════════════════════════
    if (pendingHITLForm) {
      console.log("[HITL] User typed in chat while form pending - sending as 'response' rejection");
      ...
      // Send as response type (user's message replaces form interaction)
  ...
  const handleHITLFormSubmit = useCallback(async (
    ...
    // Format the form response as a message to show the user
    const formResponse = Object.entries(formData)
      .map(([key, value]) => `${key}: ${value}`)
      .join("\n");
    
    // Add user message showing their form input
    const turnId = `turn-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
    addMessage(activeConversationId, { 
      role: "user", 
      content: `**Form Response (${decisionType}):**\n\n${formResponse}` 
    }, turnId);
  ...
  const handleHITLFormCancel = useCallback(async () => {
    ...
    addMessage(activeConversationId, { 
      role: "user", 
      content: "I cancelled the form request." 
    }, turnId);
            ...
            >
              ...
              <div className="flex-1 max-w-full">
                ...
                <MetadataInputForm
                  ...
                  onSubmit={(data) => handleHITLFormSubmit(data, 'approve')}
