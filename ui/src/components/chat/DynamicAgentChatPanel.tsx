...
export function ChatPanel({ endpoint, conversationId, conversationTitle, readOnly, readOnlyReason, agentId, agent, isLoadingMessages, clientContext: extraClientContext, suggestedPrompts, suggestedPromptsInitiallyHidden = false, emptyStateTitle, emptyStateSubtitle, surface = "default", fontScale = "default" }: ChatPanelProps) {
  ...
  }, [isThisConversationStreaming, activeConversationId, accessToken, agentId, agentProtocol, createConversation, clearStreamEvents, addMessage, appendToMessage, updateMessage, setConversationStreaming, buildStreamCallbacks, finalizeStreamLoop, session?.user, showAuthErrorToast, toast, extraClientContext]);
  ...
  const handleUserInputSubmitSSE = useCallback(async (formData: Record<string, string>) => {
    ...
    try {
      ...
      appendToMessage, setConversationStreaming, clearStreamEvents, buildStreamCallbacks, finalizeStreamLoop, getActiveConversation, extraClientContext]);
