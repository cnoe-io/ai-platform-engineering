  ...
  const submitMessage = useCallback(async (messageToSend: string) => {
    ...
    try {
      ...
      if (!(error as Error).message?.startsWith("Session expired:")) {
        ...
        const userMessage = raw.includes("Failed to fetch") || raw.includes("NetworkError") || raw.includes("ECONNREFUSED")
          ? "The agent backend is not reachable. Make sure an LLM provider is configured and the Custom Agents service is running."
          : raw || "Failed to connect to agent endpoint";
        updateMessage(convId!, assistantMsgId, {
          content: userMessage,
          isFinal: true,
          isError: true,
