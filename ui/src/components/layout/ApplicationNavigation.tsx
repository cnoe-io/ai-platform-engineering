...
function activeAreaForPath(
  pathname: string | null,
  pinnedAppItems: ApplicationNavigationItem[] = [],
): string | null {
  ...
  const activePinnedApp = pinnedAppItems
    .filter((item) => pathname === item.href || pathname?.startsWith(`${item.href}/`))
    .sort((a, b) => b.href.length - a.href.length)[0];
  if (activePinnedApp) return activePinnedApp.key;
...
function ApplicationNavigationContents({
  ...
  const [pinnedAppItems, setPinnedAppItems] = React.useState<ApplicationNavigationItem[]>([]);
  const [topNavConfig, setTopNavConfig] = React.useState<TopNavConfig | null>(null);

  React.useEffect(() => {
    if (!(storageMode === "mongodb" && config.agenticAppsEnabled)) {
      setPinnedAppItems([]);
      return;
    }
    let cancelled = false;
    fetch("/api/agentic-apps")
      ...
      .then((body) => {
        ...
        const apps = (body.items ?? body.data?.items ?? []) as Array<{
          appId: string;
          href?: string;
          displayName?: string;
        ...
        setPinnedAppItems(
          apps
            .filter(
              (app) => app?.surfaces?.showInTopNav === true && typeof app.href === "string",
            )
            .map((app) => ({
              key: `app-${app.appId}`,
              href: app.href as string,
  ...
  React.useEffect(() => {
    const fetchTopNavConfig = () =>
      fetch("/api/admin/platform-config")
        .then((response) => (response.ok ? response.json() : null))
        .then((body) => setTopNavConfig(normalizeTopNavConfig(body?.data?.top_nav)))
        .catch(() => undefined);
    fetchTopNavConfig();
    window.addEventListener("caipe:top-nav-config-updated", fetchTopNavConfig);
    return () => window.removeEventListener("caipe:top-nav-config-updated", fetchTopNavConfig);
  }, []);

  const activeArea = activeAreaForPath(pathname, pinnedAppItems);
  ...
  ].filter(Boolean) as ApplicationNavigationItem[],topNavConfig);
