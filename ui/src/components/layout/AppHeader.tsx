...
export function getApplicationBreadcrumbs(
  ...
  const sectionLabel =
    section === "projects"
      ...
      : APPLICATION_SECTION_LABELS[section] ?? titleCaseRouteSegment(section);
  ...
  if (section !== "apps") return breadcrumbs;

  const appId = segments[1];
  if (!appId) return breadcrumbs;
  ...
  const appBreadcrumb: WorkspaceBreadcrumbItem = {
    label: appId === "agentic-sdlc" ? "Agentic SDLC" : titleCaseRouteSegment(appId),
    href: `/apps/${encodeURIComponent(appId)}`,
  };
  breadcrumbs.push(appBreadcrumb);

  if (appId !== "agentic-sdlc") {
    if (segments.length > 2) {
      breadcrumbs.push({
        label: segments.slice(2).map(titleCaseRouteSegment).join(" / "),
      });
    }
    return breadcrumbs;
  }
  ...
  if (!owner || !repo) return breadcrumbs;
  ...
  const epicIndex = segments.indexOf("epics", 4);
  const epicId = epicIndex >= 0 ? segments[epicIndex + 1] : undefined;
  if (epicId) breadcrumbs.push({ label: titleCaseRouteSegment(epicId) });
