 ...
 *
 * Combines:
 *   1) An initial `GET /api/agentic-sdlc/repos/{owner}/{repo}/epics/{epicId}`
...
function detailUrl(owner: string, repo: string, epicId: string): string {
  return `/api/agentic-sdlc/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/epics/${encodeURIComponent(epicId)}`;
}
...
function replaceOrAppend(
  list: AgenticSdlcArtifact[],
  next: AgenticSdlcArtifact,
): AgenticSdlcArtifact[] {
...
function bucketFor(
  state: EpicShipState,
  kind: AgenticSdlcArtifact["kind"],
): AgenticSdlcArtifact[] | null {
...
export function useEpicShipState({
    ...
    (msg: AgenticSdlcStreamMessage) => {
      switch (msg.event) {
        ...
        case "artifact_upserted": {
          const a = msg.data as AgenticSdlcArtifact;
        ...
        case "event_appended": {
          const ev = msg.data as SafeAgenticSdlcEvent;
  ...
  const stream = useAgenticSdlcStream({
