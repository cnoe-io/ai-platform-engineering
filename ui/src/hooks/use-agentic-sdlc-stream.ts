...
/**
 * Generic SSE subscriber for Agentic SDLC streams.
 *
...
export type AgenticSdlcStreamEvent =
  | "connected"
  | "artifact_upserted"
  | "event_appended"
  | "stage_transition"
...
export interface AgenticSdlcStreamMessage<T = unknown> {
  event: AgenticSdlcStreamEvent;
  data: T;
}

export type AgenticSdlcStreamStatus =
  | "idle"
  | "connecting"
  | "open"
  | "reconnecting"
...
interface UseAgenticSdlcStreamOptions {
  /**
   * Full path to the SSE endpoint, e.g.
   * `/api/agentic-sdlc/repos/owner/repo/epics/I_42/events`. Pass null
   ...
   */
  onEvent: (msg: AgenticSdlcStreamMessage) => void;
  /**
...
interface UseAgenticSdlcStreamReturn {
...
const KNOWN_EVENTS: AgenticSdlcStreamEvent[] = [
  "connected",
  "artifact_upserted",
  "event_appended",
  "stage_transition",
...
export function useAgenticSdlcStream({
  url,
  onEvent,
  disableReconnect,
}: UseAgenticSdlcStreamOptions): UseAgenticSdlcStreamReturn {
  const [status, setStatus] = useState<AgenticSdlcStreamStatus>("idle");
  ...
  const connect = useCallback(
    (currentUrl: string, attempt: number) => {
      ...
      const handle = (name: AgenticSdlcStreamEvent) => (raw: MessageEvent) => {
