...
describe("agentic app registry", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.AGENTIC_APP_OSS_REPO_MANAGEMENT_ORIGIN;
    delete process.env.AGENTIC_APP_OSS_REPO_MANAGEMENT_MOUNT_PATH;
    delete process.env.AGENTIC_APP_OSS_REPO_MANAGEMENT_DISABLED;
    delete process.env.AGENTIC_APP_JIRA_PROJECT_DASHBOARD_ORIGIN;
    delete process.env.AGENTIC_APP_JIRA_PROJECT_DASHBOARD_MOUNT_PATH;
    delete process.env.AGENTIC_APP_JIRA_PROJECT_DASHBOARD_DISABLED;
  ...
  it("loads the external Agentic SDLC reference app for the Apps Hub when SHIP_LOOP_ENABLED=true", async () => {
    process.env.AGENTIC_APPS_INSTALL_ENABLED = "true";
  ...
  it("loads OSS Repo Management and Jira Project Dashboard from host configuration", async () => {
    process.env.AGENTIC_APPS_INSTALL_ENABLED = "true";
    process.env.AGENTIC_APPS_ENABLED = "oss-repo-management,jira-project-dashboard";
    process.env.AGENTIC_APP_OSS_REPO_MANAGEMENT_ORIGIN = "http://localhost:3040";
    process.env.AGENTIC_APP_JIRA_PROJECT_DASHBOARD_ORIGIN = "http://localhost:3041";
    ...
    expect(getAgenticAppById("oss-repo-management")?.health.endpoint).toBe("/healthz");
    expect(getAgenticAppById("jira-project-dashboard")?.health.endpoint).toBe("/healthz");
    expect(getAgenticAppById("project-management")).toBeNull();
  });
