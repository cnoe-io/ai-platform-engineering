 ...
 * stream into a single state object. We mock useAgenticSdlcStream so
...
describe("useEpicShipState", () => {
  it("seeds state from the GET response and exposes the stream status", async () => {
    ...
    expect((global.fetch as jest.Mock).mock.calls[0][0]).toBe(
      "/api/agentic-sdlc/repos/demoorg/agentic-demo/epics/I_42",
    );
  });
  ...
  it("flags a terminal error from the stream and does not apply later events", async () => {
    ...
    // useAgenticSdlcStream because terminal flips. We assert via the
