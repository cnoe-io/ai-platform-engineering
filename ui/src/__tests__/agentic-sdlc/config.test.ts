/**
 * Verify that the two Agentic SDLC config knobs round-trip cleanly through
...
describe("Agentic SDLC config keys", () => {
  ...
  it("reads SHIP_LOOP_ASSISTANT_ENABLED=true independent of the parent flag", () => {
    ...
    // (`shipLoopEnabled`) is enforced by `useAgenticSdlcFeature` and
    // `withAgenticSdlcGate`, not by mutating the assistant flag here.
    expect(cfg.shipLoopEnabled).toBe(false);
