...
describe("GET /api/ship-loop/repos/{owner}/{repo}/epics/{epicId}", () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const { GET } = await import(
      "@/app/api/ship-loop/repos/[owner]/[repo]/epics/[epicId]/route"
    );
    const res = await GET(
      new Request("http://localhost/api/ship-loop/repos/x/y/epics/I_1"),
  ...
  it("returns 404 when Epic does not exist", async () => {
    ...
    const res = await GET(
      new Request(
        "http://localhost/api/ship-loop/repos/demoorg/agentic-demo/epics/I_missing",
      ),
      {
  ...
  it("buckets children by kind and computes needs_me from caller email", async () => {
    ...
    const res = await GET(
      new Request(
        "http://localhost/api/ship-loop/repos/demoorg/agentic-demo/epics/I_42",
      ),
      {
        params: Promise.resolve({
          owner: "demoorg",
