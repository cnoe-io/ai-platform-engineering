 ...
 *
 * Tests for the Ship Loop server-side toggle gate.
 *
 * These are the security boundary: when the feature is disabled, every
 * /api/ship-loop/** route MUST return 404 (not 403/500) and MUST NOT
...
import { withShipLoopGate, isShipLoopServerEnabled } from "@/lib/ship-loop/guard";
import type { NextRequest } from "next/server";

describe("withShipLoopGate", () => {
  ...
  function makeReq(): NextRequest {
    return { url: "http://localhost/api/ship-loop/repos" } as unknown as NextRequest;
  }

  it("returns 404 with no body when SHIP_LOOP_ENABLED is unset", async () => {
    delete process.env.SHIP_LOOP_ENABLED;
    let invoked = false;
    const handler = withShipLoopGate(async () => {
      invoked = true;
  ...
  it("isShipLoopServerEnabled mirrors the env state", () => {
    process.env.SHIP_LOOP_ENABLED = "true";
    expect(isShipLoopServerEnabled()).toBe(true);
    process.env.SHIP_LOOP_ENABLED = "false";
    expect(isShipLoopServerEnabled()).toBe(false);
    delete process.env.SHIP_LOOP_ENABLED;
    expect(isShipLoopServerEnabled()).toBe(false);
  });
});
