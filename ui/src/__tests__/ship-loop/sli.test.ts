 ...
 */
import {
  __resetShipLoopSlisForTests,
  decSseConnection,
  incSseConnection,
  recordProjectionLatency,
  recordWebhook,
  setWorkerQueueDepth,
  snapshotShipLoopSlis,
} from "@/lib/ship-loop/sli";

describe("Ship Loop SLIs", () => {
  beforeEach(() => __resetShipLoopSlisForTests());

  it("records webhook outcomes by label", () => {
    recordWebhook("accepted");
    recordWebhook("accepted");
    recordWebhook("rejected_signature");
    expect(snapshotShipLoopSlis().webhooks).toEqual({
      accepted: 2,
  ...
  it("setWorkerQueueDepth ignores invalid input", () => {
    ...
    expect(snapshotShipLoopSlis().worker_queue_depth).toBe(5);
  });
  ...
  it("inc/decSseConnection clamps at zero", () => {
    ...
    decSseConnection();
    decSseConnection();
    expect(snapshotShipLoopSlis().sse_active_connections).toBe(0);
  });
  ...
  it("records projection latency into bucket histogram", () => {
    ...
    const snap = snapshotShipLoopSlis().projection_latency;
  ...
  it("ignores non-finite or negative latency", () => {
    ...
    expect(snapshotShipLoopSlis().projection_latency.count).toBe(0);
  });
});
