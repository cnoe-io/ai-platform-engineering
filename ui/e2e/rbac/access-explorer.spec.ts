...
test.describe("mocked Access Explorer browser regression", () => {
  ...
  test("searches actors and resource entities before rendering effective access", async ({
    ...
    await page.getByRole("textbox", { name: "Search reachable agents..." }).fill("github");
    await page.getByRole("option", { name: "GitHub Agent" }).click();
    await expect(featureAgentPicker).toContainText("GitHub Agent");
