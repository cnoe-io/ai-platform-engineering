...
const DOMAIN_RULES = [
  {
    id: "DEP-01",
    title: "Deployment, upgrade, and runtime health",
  ...
  },
  {
    ...
    tests: ["TOME-01", "TOME-02", "TOME-03", "TOME-04"],
    patterns: [
...
function parseArgs(argv) {
  const values = { base: process.env.GRID_TAP_RELEASE_BASE_REF || "0.5.0", head: process.env.GRID_TAP_RELEASE_HEAD_REF || "HEAD", format: "json" };
...
function summary(report) {
  const lines = [
    `GRID TAP release range: ${report.base.ref} (${report.base.sha.slice(0, 12)})..${report.head.ref} (${report.head.sha.slice(0, 12)})`,
...
try {
  ...
  process.stderr.write(`GRID TAP release matrix failed: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 2;
}
