...
Webhook provider
   ...
   Public webhook gateway / verifier
        │
        ▼  verified delivery
   AWS SNS (fan-out topic)
        │
        ▼  raw subscription
   Dedicated AWS SQS queue (one per consumer)
        │
...
| `SQS_QUEUE_NAME` | `webhook-deliveries` | Dedicated queue name (not URL). Do not share it with another consumer. |
| `CAIPE_WEBHOOK_URL` | `http://caipe-ui:3000/api/webhooks/github` | Where the receiver POSTs forwarded payloads. |
...
SQS_QUEUE_NAME=webhook-deliveries \
  CAIPE_WEBHOOK_URL=http://localhost:3000/api/webhooks/github \
...
```bash
docker compose -f docker-compose.dev.yaml --profile sqs-webhook up sqs-webhook-proxy-receiver
```

The service is gated behind the `sqs-webhook` profile so it only spins up
