...
# Run the LiteLLM MCP server locally for Grid/dynamic-agent testing
...
### LiteLLM MCP Local Dev

The LiteLLM MCP server is a standalone MCP service for dynamic-agent testing.
It does not require a CAIPE persona or A2A agent container.

Set the shared LiteLLM token in the project-root `.env` file:
...
Use `http://localhost:18080/mcp/` from the host machine. If a local FinOps
agent is running in the same Docker Compose network, use
