...
{{- define "keycloak.evaluatorOboSecretName" -}}
{{- if .Values.evaluatorTokenExchange.secretRef }}
{{- .Values.evaluatorTokenExchange.secretRef }}
{{- else }}
{{- $base := include "keycloak.fullname" . | trunc 46 | trimSuffix "-" -}}
{{- printf "%s-evaluator-obo" $base -}}
{{- end }}
{{- end }}

{{- define "keycloak.evaluatorTokenExchangeEnabled" -}}
{{- $featureEnabled := dig "evaluator" "enabled" true (.Values.global | default dict) -}}
{{- if hasKey .Values.evaluatorTokenExchange "enabled" -}}
{{- and $featureEnabled .Values.evaluatorTokenExchange.enabled -}}
{{- else -}}
{{- and $featureEnabled .Values.tokenExchange.enabled -}}
{{- end -}}
{{- end -}}

{{- define "keycloak.shouldCreateEvaluatorOboSecret" -}}
{{- if and (eq (include "keycloak.evaluatorTokenExchangeEnabled" .) "true") (not .Values.evaluatorTokenExchange.secretRef) -}}
true
{{- end -}}
{{- end -}}
