...
The current runner authenticates with a shared scheduler token and sends owner context through headers/body. That is enough for the UI/BFF to recognize the request, but it is not enough for Dynamic Agents because the DA authz gate expects a validated bearer token. The failure currently appears as:
...
1. Cron runner authenticates as `service_account:caipe-scheduler-runner`.
2. Runner sends only `schedule_id` and run metadata.
3. UI/Scheduler loads the schedule from DB and resolves the immutable owner.
4. The platform asks Keycloak for an owner-user token using token exchange / impersonation.
5. UI proxies to DA with `Authorization: Bearer <owner-user-token>`.
6. UI/DA also pass `client_context.source=scheduler`, `schedule_id`, and `run_id`.
...
- The runner sends `schedule_id`, not trusted owner identity.
- The owner is resolved only from the schedule DB record.
- The scheduler service account is scoped only for scheduled execution token exchange.
- All runs persist `source=scheduler`, `schedule_id`, `run_id`, and actor service account metadata.
