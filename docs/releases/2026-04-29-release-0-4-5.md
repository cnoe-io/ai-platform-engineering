...
> Released: 2026-04-29
> Chart: `oci://ghcr.io/caipe-io/charts/ai-platform-engineering:0.4.5`
...
- **rag-server**: install `chromium_headless_shell` in `Dockerfile.ingestors` (x86_64); add init container chart support — fixes JS rendering failures introduced in the 0.4.4 image ([#1320](https://github.com/caipe-io/ai-platform-engineering/pull/1320))
- **ui**: remove `$facet` aggregation from conversations and audit log APIs for CosmosDB/DocumentDB compatibility; add `client_type=webui` default filter so Slack conversations no longer appear in web UI sidebar ([#1321](https://github.com/caipe-io/ai-platform-engineering/pull/1321))
- **ui**: fix dynamic agent chat `/skills` slash command not loading hub-sourced skills — now uses the unified `/api/skills` catalog endpoint ([#1314](https://github.com/caipe-io/ai-platform-engineering/pull/1314))
- **slack-bot**: fix only the first matched agent firing for a mention; wire `overthink_config` so agents with `overthink: enabled: true` correctly gate on `@mentions` ([#1315](https://github.com/caipe-io/ai-platform-engineering/pull/1315))
- **docs**: fix broken link in 0.3.x-to-0.4.0 migration guide ([#1316](https://github.com/caipe-io/ai-platform-engineering/pull/1316))
