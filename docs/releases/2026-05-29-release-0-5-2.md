...
- **CRD-free routing mode** — new `global.agentgateway.routingMode` lets operators choose `static` to render the standalone proxy's config (one `/mcp/<id>` route/backend per target) instead of Gateway API/AgentGateway custom resources. `static` installs cleanly on clusters without the `gateway.networking.k8s.io`/`agentgateway.dev` CRDs or a controller, keeping `helm diff`/`helm upgrade` clean; MCP endpoints stay discoverable via the proxy admin `/config` endpoint consumed by the UI discover/sync flow ([#1629](https://github.com/cnoe-io/ai-platform-engineering/pull/1629))
