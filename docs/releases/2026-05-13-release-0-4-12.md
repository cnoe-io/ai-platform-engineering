...
> Released: 2026-05-13
> Chart: `oci://ghcr.io/caipe-io/charts/ai-platform-engineering:0.4.12`
...
### LiteLLM FinOps MCP
- **`mcp-litellm` server** — new standalone MCP server (`ghcr.io/caipe-io/mcp-litellm`) exposing LiteLLM FinOps reporting tools for cost and usage analysis; enable it when dynamic agents should call LiteLLM tools directly. Ships with theme-aware FinOps workflow images and improved LiteLLM FinOps report exports. Deployed via the new optional `litellmMcp` Helm section (`enabled: false` by default) ([#1402](https://github.com/caipe-io/ai-platform-engineering/pull/1402))

### Dynamic Agents
- **Agent ID in self-identity** — the `self_identity` built-in tool now returns the persisted dynamic agent ID, so an agent can reliably report which agent it is ([#1403](https://github.com/caipe-io/ai-platform-engineering/pull/1403))

## Security

- **litellm**: sanitize example values that resembled secrets in the LiteLLM MCP configuration so shipped examples no longer look like real API keys ([#1402](https://github.com/caipe-io/ai-platform-engineering/pull/1402))
...
| `litellmMcp.image.repository` | `ghcr.io/caipe-io/mcp-litellm` | Server image |
