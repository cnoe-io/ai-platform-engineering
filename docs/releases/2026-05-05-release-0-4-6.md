...
> Released: 2026-05-05
> Chart: `oci://ghcr.io/caipe-io/charts/ai-platform-engineering:0.4.6`
...
- **Upgrade detection** — re-runs skip redundant prompts by detecting the existing deployment state; EKS node kubeconfig symlinks detected and offered a writable replacement ([#1336](https://github.com/caipe-io/ai-platform-engineering/pull/1336))
...
- **Reduced retry latency** — `Retry-After` reduced from 10s to 5s; runtime TTL reduced to 60s for faster cleanup ([multiple commits](https://github.com/caipe-io/ai-platform-engineering/compare/0.4.5...0.4.6))

## Bug Fixes

- **ui**: sync agents tab selection to URL `?tab=` param — tabs are now linkable and bookmarkable; include agent name in editor card title ([#1325](https://github.com/caipe-io/ai-platform-engineering/pull/1325))
