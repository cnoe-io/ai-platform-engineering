...
> Released: 2026-05-29
> Chart: `oci://ghcr.io/caipe-io/charts/ai-platform-engineering:0.5.4`
...
### Admin UI
- **Shared pickers and connector panels** — native agent/team `<select>` elements are replaced with the shared searchable picker components, and the Slack (`SlackChannelRebacPanel`) and Webex (`WebexSpaceRebacPanel`) admin panels are DRYed up into one shared `ConnectorAdminPanel`, giving both connectors identical ReBAC management UX ([#1632](https://github.com/caipe-io/ai-platform-engineering/pull/1632))
- **Team card resource counts** — team cards now show knowledge-base, agent, and tool counts so admins can see a team's footprint without opening it ([#1643](https://github.com/caipe-io/ai-platform-engineering/pull/1643))

## Bug Fixes

- **admin**: point the team Knowledge tab create links at `/knowledge-bases` instead of a stale route ([#1643](https://github.com/caipe-io/ai-platform-engineering/pull/1643))
- **keycloak**: render the configured realm name instead of the hardcoded `caipe` default ([#1657](https://github.com/caipe-io/ai-platform-engineering/pull/1657))
- **compose**: remove a duplicate `volumes` key in the `openfga-init` service that broke `docker compose` parsing ([#1638](https://github.com/caipe-io/ai-platform-engineering/pull/1638))
