...
| Spec 104 — BFF writes slug-keyed `team_member:<slug>` realm roles when admins add/remove team members (matches AGW CEL exactly) | `ui/src/app/api/admin/teams/[id]/members/route.ts` (`syncKeycloakTeamRole`) |
