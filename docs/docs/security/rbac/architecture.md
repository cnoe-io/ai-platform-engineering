...
- `init-idp.sh` (runs in the `keycloak-init` job; seeds demo personas only when `KEYCLOAK_SEED_DEMO_USERS=true`; it does not create CAIPE realm roles). The same idempotent script also enables the `caipe-ui` service account and adds the `slack-bot-admin-audience` and `webex-bot-admin-audience` mappers so the Web UI backend can mint client-credentials tokens for the Slack and Webex bot admin APIs.
- Slack/Webex bot onboarding also repairs the Keycloak OBO prerequisites that are easy to miss in local/dev realms: the bot-specific token-exchange policy must be attached to the bot client token-exchange permission, the realm users `impersonate` permission, and the `agentgateway` target-audience token-exchange permission. The repair code merges existing `associatedPolicies` before writing so Slack and Webex policies do not overwrite each other; `agentgateway` must also have only the selected `team-<slug>` default scope when minting a team-scoped bot OBO token.
...
- **Organization access** is `user:<sub> member|admin|auditor organization:<org_key>` or team-mediated variants.
...
### 0.5.1 Transitional Gates and Retirement Plan

The 0.5.1 release branch intentionally contains a few duplicate gates while
traffic is moving from legacy service-local authorization toward BFF/OpenFGA
ownership. Treat these as migration guardrails, not the target architecture:

| Area | Current gate | Why it exists in 0.5.1 | Retirement condition |
|------|--------------|------------------------|----------------------|
| Dynamic Agents identity | Web UI BFF sends both `Authorization: Bearer <token>` and legacy `X-User-Context`; Dynamic Agents validates Bearer tokens when present. | Allows rollout without breaking older direct callers while DA starts forwarding verified user tokens to AgentGateway. | Set `DA_REQUIRE_BEARER=true` in managed installs, verify no caller depends on `X-User-Context`, then remove the legacy DA identity path. Tracked by [#1454](https://github.com/cnoe-io/ai-platform-engineering/issues/1454). |
| Dynamic Agents execution authz | BFF gates v1 chat start/invoke/resume/cancel and DA repeats `agent#can_use` for runtime start/invoke/resume. | Defense-in-depth for direct DA entry points during the migration. | Decide which callers are allowed to bypass the BFF. Keep a service-side check only for direct callers; avoid double PDP hops for BFF-only paths. Tracked by [#1454](https://github.com/cnoe-io/ai-platform-engineering/issues/1454). |
| Slack/Webex runtime ReBAC | Bots delegate channel/space access decisions to BFF access-check APIs; admin diagnostic routes remain separately gated. | Keeps OpenFGA policy logic in the BFF while bots remain transport clients. | Runtime bot checks use non-admin integration routes; operator diagnostics continue using admin routes. Tracked by [#1455](https://github.com/cnoe-io/ai-platform-engineering/issues/1455). |
| Slack bot identity/team lookup | Slack bot still has Keycloak Admin and MongoDB paths for user linking and channel-team resolution. | Preserves the existing JIT/linking and channel mapping flow while the BFF integration endpoints are introduced. | Move identity and channel-team resolution behind BFF endpoints, then remove Slack bot Keycloak Admin and MongoDB runtime credentials. Tracked by [#1456](https://github.com/cnoe-io/ai-platform-engineering/issues/1456). |
| RAG datasource authz | BFF constrains UI requests and RAG repeats token/OpenFGA/Mongo-derived datasource checks for direct requests. | Protects non-BFF API/MCP paths while datasource ReBAC is being backfilled. | Keep explicit OpenFGA checks for direct clients, remove Mongo/team ownership reads from query hot paths, and avoid duplicate checks for BFF-constrained UI requests. Tracked by [#1457](https://github.com/cnoe-io/ai-platform-engineering/issues/1457). |
...
Conversations use a hybrid ownership model to avoid creating high-cardinality owner tuples for every private chat. Private ownership is implicit from MongoDB (`owner_subject` for normalized records, legacy `owner_id` email fallback for old records). Explicit OpenFGA relationships remain the enforcement store for cross-boundary sharing and admin surfaces. The Web UI backend applies the same implicit-or-explicit conversation check on chat list/detail routes, Dynamic Agent v1 stream/invoke/resume/cancel proxy routes, and conversation metadata updates. This lets Slack OBO requests write their own thread conversations and bookkeeping metadata without requiring explicit owner tuples. The Admin → System → Migrations tab runs the 0.5.1 release migrations, including `conversation_owner_identity_v1` for `owner_subject`/`owner_identity_version=2`, universal team-resource OpenFGA backfill, Dynamic Agent tool tuple reconciliation, Dynamic Agent organization-admin inheritance backfill, Slack channel and Webex space ReBAC grant backfills, messaging team mapping reconciliation, RBAC index creation, and Webex messaging ReBAC index creation. Migration runs are recorded in `schema_migrations`, and collection schema state is recorded in `data_schema_versions`.
...
NextAuth holds the refresh token and silently refreshes the access token before it expires. The bundled Keycloak realm keeps access tokens at 1 hour and sets the SSO idle timeout to 4 hours, so browser refresh tokens outlive the access token refresh window. If the refresh fails (revoked session, Keycloak down), the user is redirected to login. The access token in the session is always the current live token — it's what gets forwarded to backend services.
