...
- **Login bootstrap access** is repaired on each successful CAIPE login. If the user passes `OIDC_REQUIRED_GROUP`, the Web UI BFF writes the minimum durable baseline tuples `user:<sub> member organization:<org_key>` and `user:<sub> reader system_config:platform_settings`. If the user also matches `OIDC_REQUIRED_ADMIN_GROUP` or `BOOTSTRAP_ADMIN_EMAILS`, login bootstrap also writes `admin organization:<org_key>` and `manager system_config:platform_settings`. This is an OpenFGA reconciliation step, not a runtime realm-role fallback; users who fail the OIDC admission group are never bootstrapped.
...
`GET /api/admin/users` exposes raw Keycloak protocol roles for platform-admin diagnostics. Non-admin callers receive a self-scoped response containing only their own Keycloak user row, so the baseline Users tab can show "my access" without leaking other users. Product authorization should be read through teams and OpenFGA relationships. Local/dev realms can remove stale legacy CAIPE roles with `scripts/cleanup-local-keycloak-legacy-roles.py`.
