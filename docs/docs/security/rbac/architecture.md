...
**`data_source` and `mcp_tool` OpenFGA types + reconcilers + BFF list filter (PR 4, 2026-05-27).** `deploy/openfga/model.fga` introduces two new types:
...
Routes that have not yet been rewritten inline no longer remain session-only: the deprecated `withAuth()` compatibility wrapper now uses `getAuthFromBearerOrSession()`, resolves the route family to a least-privilege RBAC policy, and calls `requireRbacPermission()` before invoking the handler. The compatibility resolver now maps common self-service and runtime routes to explicit capabilities instead of the old generic `supervisor#invoke` umbrella: profile and identity-link routes use `self_profile#read/write`, user search uses `user_directory#read`, settings use `user_settings#read/write`, feedback/NPS uses `feedback#submit`, chat/A2A/model discovery uses `chat_supervisor#invoke`, files use `user_files#read/write`, AI assist uses `ai_assist#invoke`, credentials use `credential_vault#use`, and platform-config reads use `system_config#read`. Unknown `withAuth` routes fail toward `admin_ui#view/manage` so new route families are visible in audit logs instead of silently inheriting chat access.
...
| `knowledge_base:<id>`          | base `reader`, `ingestor`, `manager`; derived `can_read`, `can_ingest`, `can_admin` | Team Knowledge Base assignments and **Settings → Knowledge Bases** write `reader`, `ingestor`, or `manager` relationships before persisting Mongo assignment metadata. KB pages, sharing, and KB-scoped routes check these relationships. |
