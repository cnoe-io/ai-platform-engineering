...
**Catalog API key and local-dev sessions need a `sub` for skill visibility filtering (2026-07-24 fix).** `filterSkillsByOpenFga()` short-circuits to an empty list whenever `options.subject` is unset (`if (!options.subject) return [];`), since it has no identity to resolve OpenFGA reads against. `getAuthFromBearerOrSession()`'s catalog-API-key branch and its local-skills-JWT-without-`sub` branch previously returned a `session` with no `sub` field, so `GET /api/skills` calls through either path returned zero skills — hub and global `agent_skills` included, not just the filesystem-default catalog. The fix populates `sub`: the catalog-key path gets the fixed synthetic subject `catalog-key-user@local` (which `filterSkillsByOpenFga` already special-cases to allow read-only access to every global-visibility skill — default, hub, and `agent_skills` explicitly marked `visibility: "global"` — without per-resource tuples), and the local-JWT path gets the resolved local identity's email. Neither synthetic subject is used for audit logs or ownership checks on user-owned resources; it exists solely to unblock the OpenFGA read-path lookup for global-scope catalog content.
