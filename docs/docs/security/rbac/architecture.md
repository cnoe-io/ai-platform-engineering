...
**Ownership transfer (spec 2026-06-03, US3).** Owner team is immutable on a normal edit but transferable via the editor's "Transfer ownership" affordance. The route guards a transfer with `canTransferResourceOwnership` (`ui/src/lib/rbac/resource-authz.ts`) — the caller must hold `<type>:<id>#can_manage` (owner-team admin) **or** be org admin — passes `previousOwnerTeamSlug` to the reconciler so the old owner team's grants are revoked, and requires an explicit `confirm_not_member` when the transferor is not a member of the destination team. The `creator` tuple is never touched, preserving provenance across transfers.
