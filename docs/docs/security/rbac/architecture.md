...
`can_use organization:<org_key>` in OpenFGA (granted by organization `member`
or `admin`). The Admin → Settings → Credentials tab is stricter: it is also
...
produce a default-agent OpenFGA tuple. The backfill is still the bulk repair path
...
| `KEYCLOAK_RBAC_ACTIVE_TEAM_SLUG` (`KEYCLOAK_DEFAULT_ACTIVE_TEAM_SLUG`, `ACTIVE_TEAM_SLUG`) | Team slug whose `team-<slug>` client scope sits as the *single* default `team-*` scope on `caipe-platform`. Because Keycloak's RFC 8693 token-exchange flow ignores `scope=` and only injects mappers from default audience scopes, this one scope is what stamps the `active_team` claim into every bot OBO token | Required only when MongoDB has more than one team. With exactly one team the migration auto-picks it; with multiple teams and no env var the migration emits a "Skipped active\_team default selection…" warning and OBO tokens go out without an `active_team` claim. The Admin → Security & Policy → Keycloak panel surfaces this warning with a plain-English explainer (see `usage.md`). Set it on the `caipe-ui` deployment to a real slug (e.g. `platform`); the slug must be lowercase alphanumerics with hyphens, max 63 chars |
...
1. **Keycloak**
  - `team-personal` client scope (hardcoded `active_team=__personal__`) bound as **optional** on both `caipe-slack-bot` and `caipe-webex-bot`. Provisioned by the realm-init script on every boot and re-asserted by the BFF reconciliation migration (`ensurePersonalTeamClientScope`).
  - **Known DM-mode limitation**: `team-personal` is NOT bound on the OBO audience client (`CAIPE_PLATFORM_AUDIENCE`). Keycloak's RFC 8693 token-exchange silently drops the `scope=` request parameter and only injects mappers from scopes that are *default* on the audience. Binding `team-personal` as default would clobber the real-team default that `selectAgentGatewayActiveTeamScope` selects; binding it as *optional* on the audience has been verified to have no effect on the issued token. The Keycloak invariants panel surfaces this as the advisory invariant `team_personal.dm_mode_known_limitation` (status `unknown`, remediation `manual_keycloak`) rather than as a perpetually red `default_on_obo_audience` failure. Tracked as a follow-up: either route DM token-exchange to a dedicated audience whose default scope is `team-personal`, or remove DM personal-mode handling from the bots.
  - **Audience-cardinality invariant**: at most ONE real `team-*` scope may be bound as default on `caipe-platform`. The pure invariant `audience.<client>.single_team_default` (emitted by `evaluateKeycloakInvariants`) fails red whenever the audience client has 2+ real `team-*` default scopes — `team-personal` is excluded from the count (it is covered by the DM-mode advisory above). Drift mechanism: `ensureTeamClientScope(<slug>)` binds `team-<slug>` as default; the paired `selectAgentGatewayActiveTeamScope(<slug>)` is supposed to unbind every other `team-*` default to maintain singularity, but the reconciliation migration skips that call when `KEYCLOAK_RBAC_ACTIVE_TEAM_SLUG` is unset AND Mongo has ≥2 teams. The panel surfaces the targeted heal action `Reconcile active-team scope` (typing a slug → `POST /api/admin/keycloak/active-team-scope` → `selectAgentGatewayActiveTeamScope(slug)`); persistent fix is to set `KEYCLOAK_RBAC_ACTIVE_TEAM_SLUG` so the reconciliation migration re-applies the pin every cycle.
  - `team-<slug>` client scopes (hardcoded `active_team=<slug>`) created on demand by the Web UI backend when a team is created in Mongo, and on startup auto-sync for pre-existing teams. Orphan `team-<slug>` scopes (no matching Mongo team and not the structural `team-personal`) are deleted by `deleteOrphanTeamClientScopes` during each reconciliation pass.
