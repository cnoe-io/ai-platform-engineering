...
2. The **Relationship Builder** turns guided choices into validated base OpenFGA tuples, for example `team:platform#member user agent:incident-agent`, and writes/revokes them through `/api/admin/openfga/relationship`.
3. The **Effective Access** panel runs `/api/admin/openfga/check` using the derived check relation, for example `team:platform#member can_use agent:incident-agent`, so admins can preview whether OpenFGA would allow the relationship.
