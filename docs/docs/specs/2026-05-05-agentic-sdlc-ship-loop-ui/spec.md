...
- Q: Should we adopt CopilotKit/CoAgents for the Ship Loop UI? → A: **Hybrid (Option C) using AG-UI** — keep the planned event-sourced visualizations and bespoke HITL action bar, **and** add a bottom-right Agentic SDLC Assistant bubble that reuses the existing Dynamic Agent chat UI and talks to the pinned `agentic-sdlc` CAIPE Dynamic Agent over the existing **AG-UI** protocol. The bubble lets the user ask questions about the current Agentic SDLC page without replacing the visualizations or HITL chrome. CopilotKit itself is **not** added as a dependency — AG-UI is the substrate.
...
In addition to the bespoke HITL action bar, a bottom-right **Agentic SDLC Assistant bubble** is available that reuses the existing Dynamic Agent chat UI and streams from the pinned `agentic-sdlc` CAIPE Dynamic Agent over the AG-UI protocol. The user can ask questions about the current Agentic SDLC page ("Why is this stalled?", "Summarize the last 3 PRs"), request a stage transition explanation, or have the agent draft an HITL comment that the user reviews and submits. The Dynamic Agent is given read-only route context for the active page (home, repo, or Epic) and is **not** authorized to take destructive actions on the user's behalf — every action that mutates GitHub or the loop still goes through the explicit HITL action bar with a human click.
...
**Independent Test**: A user can complete an end-to-end HITL approval (approve a PR, see the merge happen, see the sandbox deploy kick off) entirely from the Ship Loop UI on a single Epic. Separately, a user can open the Agentic SDLC Assistant bubble, ask "what's blocking this Epic?", and receive a streamed answer that cites the events the assistant used.
...
4. **Given** an Epic is open, **When** the user opens the Agentic SDLC Assistant bubble and asks a question, **Then** the pinned `agentic-sdlc` Dynamic Agent streams a grounded answer (citing the Epic's artifacts/events) over AG-UI within the same SLO as other agent chats in the app.
...
#### Agentic SDLC Assistant bubble (AG-UI Dynamic Agent)

- **FR-031**: System MUST provide a bottom-right Agentic SDLC Assistant bubble that reuses the existing Dynamic Agent chat UI and streams from the pinned `agentic-sdlc` CAIPE Dynamic Agent over the existing AG-UI protocol; the bubble MUST share session/auth state with the rest of the app and never introduce a separate identity.
- **FR-032**: System MUST scope the assistant's read context to the currently open Agentic SDLC page (home, repo, or Epic), including route context and later read-only Epic artifacts/events/webhook health where available; it MUST NOT receive raw webhook payload secrets, other repos' data, or other users' HITL data.
...
- **FR-035**: The Agentic SDLC Assistant bubble MUST be independently disable-able via a sub-toggle of the Agentic SDLC feature, so operators can disable just the assistant without disabling the visualizations or HITL chrome.
