...
- [X] T002 [P] Document new env vars in `ui/env.example`: `SHIP_LOOP_ENABLED`, `SHIP_LOOP_ASSISTANT_ENABLED`, `GITHUB_WEBHOOK_SECRET`, `SHIP_LOOP_AGENT_BOT_LOGINS`. The assistant bubble is pinned to the `agentic-sdlc` Dynamic Agent.
...
- [X] T005 [P] Create the Ship Loop directory skeleton under `ui/src/`: `app/(app)/ship-loop/`, `app/api/agentic-sdlc/`, `components/agentic-sdlc/`, `components/agentic-sdlc/visualizations/`, `hooks/`, `lib/agentic-sdlc/`, `types/agentic-sdlc.ts`. Add a `README.md` to each new component directory linking back to the spec.
...
- [X] T007 Implement `withAgenticSdlcGate(handler)` server-side middleware in `ui/src/lib/agentic-sdlc/guard.ts`. When `Config.shipLoopEnabled === false`, returns `404` with empty body. The per-user `ship_loop_enabled` pref is enforced inside individual user-facing routes / the layout (the receiver intentionally checks only the server gate, since GitHub has no per-user identity).
...
- [X] T010 Define typed wrappers around `getCollection` for the three new collections in `ui/src/lib/agentic-sdlc/mongo-collections.ts`: `getShipLoopReposCollection`, `getShipLoopEventsCollection`, `getShipLoopArtifactsCollection`. Mirrors the field shapes from `data-model.md`.
...
- [X] T013 Author `ui/src/types/agentic-sdlc.ts` with the entity types from `data-model.md`: `ShipLoopStage`, `OnboardedRepo`, `ShipLoopEvent` (including `projection_status`, `projection_attempts`), `ShipLoopArtifact`, `HitlActionPayload`, and the new `VelocityMetric` and `AgentRunRecord` types. Also exports `SHIP_LOOP_STAGES`, `DEFAULT_AGENT_LABEL_TO_STAGE`, and `SHIP_LOOP_COLLECTIONS` constants.

### GitHub integration

- [ ] T014 Implement `ui/src/lib/agentic-sdlc/github-client.ts` — server-only Octokit wrapper that takes the user's session token and exposes `getRepoPermission(user, owner, repo)`, `listUserRepos(user)`, `createWebhook(...)`, `deleteWebhook(...)`, `approvePR(...)`, `requestChangesPR(...)`, `commentPR(...)`, `retryDeployment(...)`. Includes a 5-minute in-memory permission cache.
- [ ] T015 [P] Implement `ui/src/lib/agentic-sdlc/webhook-verify.ts` — HMAC SHA-256 verification using `@octokit/webhooks`; takes raw body + per-repo secret; returns `{ valid: boolean, repoId: string }` without throwing on mismatch.
...
- [X] T017 [P] Implement the **pure** `resolveStage(events, labels, githubState, overrides)` function in `ui/src/lib/agentic-sdlc/stage-resolver.ts` per `data-model.md` "Stage resolution rules". No I/O.
...
- [X] T019 Implement the in-process bounded async worker in `ui/src/lib/agentic-sdlc/async-worker.ts` (renamed from `projection-worker.ts`). Per-repo FIFO ordering via in-memory queue, dedupes via `(repo_id, github_delivery_id)` at the receiver, projects derived state into `ship_loop_artifacts`, and emits to the in-process SSE pub/sub. Pure projection extracted into `ui/src/lib/agentic-sdlc/projector.ts` for unit testing without the Mongo driver.
...
- [X] T022 Implement `POST /api/agentic-sdlc/webhooks/github` in `ui/src/app/api/agentic-sdlc/webhooks/github/route.ts`. Reads raw body, looks up repo by `repository.id`, verifies HMAC against per-repo secret, enqueues to the async worker, returns **`202 Accepted`**. Returns `204` for unsubscribed event types, `404` for unknown repos or when the server flag is off, `401` on bad signature, `400` on malformed body.
...
- [X] T025 Implement `ui/src/lib/agentic-sdlc/sse-bus.ts` — in-process pub/sub keyed by `epic:<repo>:<epic>` and `inbox:<user>`, with bounded per-connection queues (default 256), overflow-close semantics, per-user 10-connection cap, and 25s heartbeats.
...
- [X] T027 [P] Implement four counters/gauges in `ui/src/lib/agentic-sdlc/sli.ts`: webhook outcome counter, worker queue depth gauge, projection latency histogram, SSE active connection gauge. Wired into the receiver, the worker, and the SSE bus.
- [ ] T028 [P] Expose the SLIs through the existing `useCAIPEHealth` surface (deferred to Chunk D — operator UX).

### Authz helper

- [X] T029 Implement `ui/src/lib/agentic-sdlc/authz.ts` — `resolveRepoPermission()` plus `requireRepoPermission(opts, "read"|"comment"|"admin")` returning 404 on no-access (to avoid leaking repo existence) and 403 on insufficient-permission. Backed by GitHub repo metadata via `github-client.ts`, results cached 60s per (user, repo).
...
- [~] T032 [P] [US1] **PARTIAL** — `GET /api/agentic-sdlc/repos` ships in `ui/src/app/api/agentic-sdlc/repos/route.ts` returning counts (open epics, in-flight subtasks, PRs awaiting review, deploys 24h) under the `{items: [...]}` envelope. Full GitHub-OAuth-backed visibility filter is deferred to FR-029 alongside repo authz.
- [ ] T033 [US1] Implement `POST /api/agentic-sdlc/repos` in the same `route.ts` — verifies repo read access, creates GitHub webhook (or links existing), persists `OnboardedRepo` in Mongo, returns `201`. Handles `409` on duplicate active onboarding.
- [ ] T034 [P] [US1] Implement `GET /api/agentic-sdlc/repos/{owner}/{repo}` in `ui/src/app/api/agentic-sdlc/repos/[owner]/[repo]/route.ts` — repo detail + webhook health.
- [ ] T035 [US1] Implement `DELETE /api/agentic-sdlc/repos/{owner}/{repo}` in the same route — sets `offboarded_at`, idempotent.
- [ ] T036 [P] [US1] Implement webhook health check job in `ui/src/lib/agentic-sdlc/webhook-health.ts` — periodically (every 60s in pilot) scans `OnboardedRepo` and updates `webhook_status` to `degraded` when no events received within the configured idle threshold.
...
- [X] T039 [P] [US1] `AgenticSdlcHome.tsx` now embeds the live `RepoGrid` (fetches `/api/agentic-sdlc/repos`) between the hero animation and the stage tile legend. Empty / loading / error states are explicit and the error path links the operator at the SHIP_LOOP_ALLOW_NO_AUTH bypass + the seed script.
- [X] T040 [US1] Wire `ShipLoopHome` into `ui/src/app/(app)/ship-loop/page.tsx` (server component delegating to the client component).
- [ ] T041 [P] [US1] Implement `OnboardRepoDialog.tsx` in `ui/src/components/agentic-sdlc/OnboardRepoDialog.tsx` — repo dropdown sourced from `listUserRepos`, sandbox-environment text input, label-mapping override JSON editor, submit calls `POST /api/agentic-sdlc/repos`.
- [ ] T042 [P] [US1] Implement `WebhookHealthBanner.tsx` in `ui/src/components/agentic-sdlc/WebhookHealthBanner.tsx` with one-click "Reconnect" calling a server action that recreates the webhook.
...
- [X] T046 [P] [US2] `GET /api/agentic-sdlc/repos/{owner}/{repo}/epics/{epicId}` returns the bundle (epic, subtasks, pull_requests, deploys, recent_events, needs_me). Strips `payload` and `_id` from `recent_events` before emission.
...
- [X] T049 [P] [US2] `EpicView.tsx` shell ships in `ui/src/components/agentic-sdlc/EpicView.tsx` with header (StageBadge, GitHub link, "needs you" callout, live-stream status indicator) and Pipeline / Kanban / Timeline tabs.
...
- [X] T053 [P] [US2] `StageBadge.tsx` (in `ui/src/components/agentic-sdlc/visualizations/`) and the `ArtifactCard.tsx` agent-vs-human iconography helper satisfy FR-016. Agent inference uses `agent_labels.length > 0` from the projector.
...
- [X] T058 [P] [US2] `GET /api/agentic-sdlc/repos/{owner}/{repo}/epics` ships with `stage=` (validated against the closed enum -> 400 on typo), `needs_human=`, `stalled=`, and cursor pagination keyed on `(last_event_at desc, _id desc)` with strict-less tie breaking on identical timestamps.
...
- [ ] T060 [P] [US3] Implement portfolio aggregation query in `ui/src/lib/agentic-sdlc/portfolio-query.ts` — uses `repo_stage_index` and `stalled_index` from `mongodb-migration.md`. Returns counts per stage, per repo.
- [ ] T061 [P] [US3] Implement `GET /api/agentic-sdlc/portfolio` in `ui/src/app/api/agentic-sdlc/portfolio/route.ts` for the dashboard payload (filters: repo[], owner, stage, needs_me, stalled).
...
- [ ] T063 [P] [US3] Implement `PortfolioDashboard.tsx` in `ui/src/components/agentic-sdlc/PortfolioDashboard.tsx` — count tiles, stalled callouts, filters bar.
- [ ] T064 [US3] Implement filter persistence in `ship-loop-store.ts` (T055) and synchronize URL query params for shareable views.
- [ ] T065 [P] [US3] Implement `NeedsYouInbox.tsx` in `ui/src/components/agentic-sdlc/NeedsYouInbox.tsx` and the SSE channel `GET /api/agentic-sdlc/needs-you` per `contracts/sse-channels.md`.
- [ ] T066 [US3] Implement `ui/src/app/api/agentic-sdlc/needs-you/route.ts` SSE handler emitting `connected`, `inbox_initial`, `inbox_added`, `inbox_removed`, `heartbeat` events.
...
- [ ] T067 [P] [US4] Implement `DependencyGraphView.tsx` (mode D) in `ui/src/components/agentic-sdlc/visualizations/DependencyGraphView.tsx` using `@xyflow/react`. Nodes: Epic, sub-tasks, PRs, deploys; edges: parentage. Color-coded by stage and `needs_human`.
- [ ] T068 [P] [US4] Implement `ShipLoopRadarView.tsx` (mode E) in `ui/src/components/agentic-sdlc/visualizations/ShipLoopRadarView.tsx` — four-quadrant SVG (Specify / Execute / Verify / Deliver+Observe) with token positions.
- [ ] T069 [P] [US4] Implement `HeatmapView.tsx` (mode F) in `ui/src/components/agentic-sdlc/visualizations/HeatmapView.tsx` for the **portfolio** view — repo × stage grid colored by activity. Wired into `PortfolioDashboard` (US3) as an optional layout.
...
## Phase 7: User Story 5 — HITL actions inline + AG-UI assistant bubble (Priority: P2)

**Goal**: Approve / request-changes / comment / retry-deploy / pause-loop from inside the UI with full audit. Plus the bottom-right **Agentic SDLC Assistant** bubble using the pinned `agentic-sdlc` CAIPE Dynamic Agent over AG-UI (per Clarification Q1).
...
- [ ] T073 [US5] Implement `POST /api/agentic-sdlc/actions/approve-pr` in `ui/src/app/api/agentic-sdlc/actions/approve-pr/route.ts` — `requireRepoTriage`, forwards to GitHub, writes a `source: "ui"` event row.
- [ ] T074 [P] [US5] Implement `POST /api/agentic-sdlc/actions/request-changes` in `ui/src/app/api/agentic-sdlc/actions/request-changes/route.ts`. `comment` field is required; sanitize before forwarding.
- [ ] T075 [P] [US5] Implement `POST /api/agentic-sdlc/actions/comment` in `ui/src/app/api/agentic-sdlc/actions/comment/route.ts`.
- [ ] T076 [US5] Implement `POST /api/agentic-sdlc/actions/retry-deploy` in `ui/src/app/api/agentic-sdlc/actions/retry-deploy/route.ts` — `requireRepoWrite`; resolves the deployment's underlying mechanism (Actions run, etc.) and triggers a retry.
- [ ] T077 [P] [US5] Implement `POST /api/agentic-sdlc/actions/pause-loop` and `.../resume-loop` in `ui/src/app/api/agentic-sdlc/actions/pause-loop/route.ts` and `.../resume-loop/route.ts` — adds/removes the `agent:paused` label on the Epic and every open child sub-task.
- [ ] T078 [P] [US5] Implement `HitlActionBar.tsx` in `ui/src/components/agentic-sdlc/HitlActionBar.tsx` with Approve / Request changes / Comment / Retry deploy / Pause+Resume buttons, gated client-side by capability flags from the API but **always re-enforced server-side**.

### Agentic SDLC Assistant bubble (Clarification Q1)

- [ ] T079 [US5] Implement `GET /api/agentic-sdlc/assistant/context/{epicId}` in `ui/src/app/api/agentic-sdlc/assistant/context/[epicId]/route.ts` — returns the read-only Epic context (artifacts, recent events, current stage, webhook health) the Dynamic Agent will consume. Strips raw webhook payload and any secrets per FR-032.
- [ ] T080 [US5] Implement assistant tool-call rejection middleware in `ui/src/lib/agentic-sdlc/assistant-guard.ts` — denies any AG-UI tool call whose target is a Ship Loop mutation route. Returns a structured rejection the agent can include in its reply.
- [X] T081 [P] [US5] Implement `AgenticSdlcAssistantBubble.tsx` in `ui/src/components/agentic-sdlc/AgenticSdlcAssistantBubble.tsx` — wraps the existing Dynamic Agent `ChatView`/`DynamicAgentChatPanel` with Agentic SDLC route context and the pinned `agentic-sdlc` dynamic agent. Hidden when `Config.shipLoopAssistantEnabled === false` or when the per-user `shipLoopAssistant` flag is off.
- [X] T082 [US5] Wire `AgenticSdlcAssistantBubble` into the Agentic SDLC layout gate as a bottom-right expandable bubble available across Agentic SDLC home, repo, and Epic pages.
...
- [ ] T086 [P] [US6] Implement event-log-derived velocity computations in `ui/src/lib/agentic-sdlc/velocity-compute.ts` (epics_merged_per_week, median_time_in_stage, agent_vs_human_pr_ratio, median_hitl_queue_age).
- [ ] T087 [US6] Implement token-spend correlation in `ui/src/lib/agentic-sdlc/token-spend.ts` — reads from existing CAIPE telemetry (Langfuse via `ui/src/lib/langfuse.ts` and/or Dynamic Agent run records); correlates runs to Epic id via the agent's working metadata. Returns total tokens, prompt/completion split, model breakdown, $-cost. **Includes a `completeness` flag** when telemetry has gaps in the requested window.
- [ ] T088 [US6] Implement `GET /api/agentic-sdlc/velocity` in `ui/src/app/api/agentic-sdlc/velocity/route.ts` — `?scope=repo|team`, `?repo=`, `?team=`, `?window=7d|30d|90d`. Calls T086 + T087. **Strips `cost_usd` fields when caller fails `requireRepoAdmin`** per FR-V08; returns the same shape so client UI can simply hide the field.
- [ ] T089 [P] [US6] Implement `VelocityPanel.tsx` in `ui/src/components/agentic-sdlc/VelocityPanel.tsx` — five tiles (epics-per-week, time-in-stage, agent-vs-human, hitl-age, token-spend) with WoW delta + sparkline + drill-through.
- [ ] T090 [P] [US6] Implement `EpicTokenSpendChip.tsx` in `ui/src/components/agentic-sdlc/EpicTokenSpendChip.tsx` — total tokens + tokens-per-merged-PR; placed in the Epic header (FR-V09).
...
- [ ] T094 [P] [US7] Implement `GET /api/agentic-sdlc/repos/{owner}/{repo}/artifacts/{artifactId}/details` in `ui/src/app/api/agentic-sdlc/repos/[owner]/[repo]/artifacts/[artifactId]/details/route.ts` — returns merged stream of GitHub events + agent progress notes + CI check results.
- [ ] T095 [P] [US7] Implement `ArtifactDetailsPanel.tsx` in `ui/src/components/agentic-sdlc/ArtifactDetailsPanel.tsx` — chronological list with check status, links to logs, links to GitHub.
...
- [ ] T097 [P] Sanitize all user/agent-supplied text fields end-to-end in `ui/src/lib/agentic-sdlc/sanitize.ts` (titles, body excerpts, comments, assistant output) using the existing markdown allow-list. Re-render tests in `ui/src/__tests__/ship-loop/sanitize.test.ts`.
- [ ] T098 [P] Add per-route rate limiting on `/api/agentic-sdlc/actions/**` (default 30/min per user-repo) and SSE-connection cap (default 10 per user) in `ui/src/lib/agentic-sdlc/rate-limit.ts`.
- [ ] T099 [P] Add audit log retention review surface in `ui/src/components/agentic-sdlc/AuditLogView.tsx` (admin-only) — read-only listing of `source: "ui"` events for a given repo.
...
- Every `/api/agentic-sdlc/**` route must use `withAgenticSdlcGate` (T007) and the appropriate `requireRepo*` helper (T029) — server-side, before any other logic.
