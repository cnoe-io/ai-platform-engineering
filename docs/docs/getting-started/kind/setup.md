...
**What is CAIPE?** CAIPE is an open-source **AI platform for platform engineering**. It provides agent creation, runtime execution, workflow automation, skills and MCP integrations, enterprise knowledge, identity and authorization, observability, and multi-channel user experiences. This setup gives you a local environment where you can try agents, add RAG (retrieval-augmented generation), and observe traces—all on your machine.
