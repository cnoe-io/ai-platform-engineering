...
|---|---|---|
| **Autonomous** (main nav) | Any user with ≥1 schedulable or automatable agent | Lists the user's own tasks, grouped by agent; creates, edits, runs and deletes them |
| **Autonomous → Automation** | Team admins (and platform admins) | Enables or disables autonomous per agent for the agent's owner team |
| **Admin → Teams & Users → Autonomous** | Organization admins | Oversight across all teams and users, with pause/resume |
| **Admin → Teams → team details** | Organization admins | Grants a team autonomous eligibility (the precondition for per-agent enablement) |

Autonomous controls no longer appear on the Agents page.

## Task identifiers

Task IDs are **generated by the server** as `slug(name)-<4 hex>` (e.g.
`daily-report-a3f9`). They are globally unique, immutable, and cannot be
chosen by the caller — an `id` supplied to `POST /api/v1/tasks` is ignored.

Display names are free text and need not be unique: two users, or one user
twice, may both have a task called "Daily report". The UI warns on a duplicate
name but does not block it.

The generated ID is what appears in the webhook URL
(`/api/v1/hooks/<task-id>`) and in run history, so it is surfaced read-only on
the task row and in the edit form.

**API callers that previously pinned IDs** (seed scripts, IaC) now receive a
generated ID instead. Idempotent seeding must match on `name` via
`GET /api/v1/tasks` rather than assuming a known ID.
...
- The CAIPE UI proxy enforces who may manage tasks. Do not bypass it for task
  management APIs. The proxy is per-user by design; the Autonomous Agents
  service applies per-task ownership using the identity headers the proxy
  injects.
