...
| Variable | Default | Allowed Values | Description |
|----------|---------|----------------|-------------|
| `DEFAULT_THEME` | `dark` | `system`, `light`, `legacy-light`, `dark`, `midnight`, `nord`, `tokyo`, `cyberpunk`, `tron`, `matrix` | Default color theme for new users. `light` selects Improved Light; `legacy-light` preserves the original bright palette. |
...
| `light` | Improved Light with balanced tonal surfaces and restrained brand color |
