...
class AIPlatformEngineerA2ABinding:
  ...
  async def stream(self, query, context_id, trace_id=None) -> AsyncIterable[dict[str, Any]]:
      ...
      # PRIORITY 2: Try to use final_ai_message first, otherwise use accumulated content
          ...
          logging.info(f"✅ Parsed response from final AIMessage: is_task_complete={final_response.get('is_task_complete')}, from_response_format_tool={final_response.get('from_response_format_tool')}")
          ...
          if final_response.get('is_task_complete') is not None:
              final_response['from_response_format_tool'] = True
          logging.info(f"✅ Parsed response from accumulated content: is_task_complete={final_response.get('is_task_complete')}, from_response_format_tool={final_response.get('from_response_format_tool')}")
      else:
