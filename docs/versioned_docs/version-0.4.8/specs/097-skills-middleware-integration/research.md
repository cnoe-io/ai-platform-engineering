...
**Implementation path**: A new FastAPI endpoint `POST /skills/scan-content` accepts `{name, content}`, materializes a single-skill temp tree, runs `skill_scanner_runner.run_scan_all_on_directory`, applies gate/threshold logic, and returns the result. The Next.js route **`ui/src/app/api/skills/configs/route.ts`** (unified from the legacy `agent-skills` tree; see `docs/docs/specs/2026-04-29-skills-api-unification/`) calls this endpoint before persisting. Scan findings are written to `skill_scan_findings` with `source_type: "agent_skills"` and `source_id` = document id (reusing the same collection as hub scan findings).
