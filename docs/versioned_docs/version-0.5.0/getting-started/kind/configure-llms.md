...
This page is **optional**. If you used [Run CAIPE with KinD](/docs/getting-started/kind/setup) and ran `./setup-caipe.sh`, the script already prompted you for an LLM provider and API key and stored them in the cluster. You can skip this page unless you need to:
...
For the standard flow, the [KinD setup guide](/docs/getting-started/kind/setup) (Step 3) is enough.
...
- **AWS Bedrock**: Use AWS credentials (env vars or `~/.aws/credentials`) or a Kubernetes secret as described in the [KinD setup](/docs/getting-started/kind/setup#aws-bedrock) section.

For full options and environment variables, see [Run CAIPE with KinD](/docs/getting-started/kind/setup).